...
class TSPredict(IStrategy):
    ...
    # ROI table:
    minimal_roi = {"0": 0.30, "30": 0.20, "60": 0.10, "80": 0.05,"100": 0.025}
...
def custom_exit(self, pair: str, trade: Trade, current_time: "datetime", current_rate: float, current_profit: float, **kwargs):
    ...
    if last_candle["volume"] <= 1.0:
        print(f"Exiting {pair} due to insufficient volume: {last_candle['volume']}")
        return None
...
def handle_short_exit(self, pair, trade, current_time, current_rate, current_profit, last_candle):
    ...
    if current_profit > 0.0:
        if last_candle["guard_metric"] <= self.cexit_metric_overbought.value:
            print(f"Covering short for {pair} as overbought guard metric reached: {last_candle['guard_metric']}")
            return "cover_overbought"

        if current_profit > 0.005:
            if last_candle["guard_metric"] <= self.cexit_metric_take_profit.value:
                print(f"Taking profit for short on {pair} as guard metric reached: {last_candle['guard_metric']}")
                return "cover_take_profit"

        if last_candle["predicted_loss"] >= last_candle["target_profit"]:
            print(f"Predicting rise for {pair}, covering short.")
            return "predict_rise"

        if last_candle["exit_short"] > 0:
            print(f"Exit signal triggered for short on {pair}")
...
def handle_long_exit(self, pair, trade, current_time, current_rate, current_profit, last_candle):
    ...
    if current_profit > 0.0:
        if last_candle["guard_metric"] >= self.cexit_metric_overbought.value:
            print(f"Selling {pair} as overbought guard metric reached: {last_candle['guard_metric']}")
            return "metric_overbought"

        if current_profit > 0.005:
            if last_candle["guard_metric"] >= self.cexit_metric_take_profit.value:
                print(f"Taking profit on {pair} as take profit guard metric reached: {last_candle['guard_metric']}")
                return "take_profit"

        if last_candle["predicted_gain"] <= last_candle["target_loss"]:
            print(f"Predicting drop for {pair}, selling for potential market drop.")
            return "predict_drop"

        if last_candle["exit_long"] > 0:
            print(f"Exit signal triggered for long on {pair}")
            return "exit_signal"
...
def check_common_exit_conditions(self, trade, current_time, current_profit):
    ...
    if num_hours >= 12 and current_profit > 0.001:
        print(f"Unclogging position held over 12 hours with profit.")
        return "unclog_12h"

    if num_days >= 1 and current_profit > 0:
        print(f"Unclogging position held over 1 day with profit.")
        return "unclog_1d"

    if num_days >= 7:
        print(f"Unclogging position held over 7 days, at a loss or profit.")
        return "unclog_7d"
