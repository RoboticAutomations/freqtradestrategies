...
class ttV8dca_CaffTech(IStrategy):

    fees_balance_notify = True
    fees_balance_coin = "BNB"
    fees_balance_notify_amount = 0.015
    fees_balance_last_checked = None
    ...
    # DISABLED - Using custom ROI stoploss.
    minimal_roi = {
        # "0": 0.084,
    ...
    # entry optizimation
    max_epa = CategoricalParameter([2, 3], default=3, space="buy", optimize=True)

    # protections
    cooldown_lookback = IntParameter(24, 48, default=29, space="protection", optimize=True)
    stop_duration = IntParameter(12, 200, default=134, space="protection", optimize=True)
    use_stop_protection = BooleanParameter(default=False, space="protection", optimize=True)

    # SMAOffset
    base_nb_candles_buy = IntParameter(5, 60, default=27, space='buy', optimize=True)
    base_nb_candles_sell = IntParameter(5, 60, default=57, space='sell', optimize=True)
    low_offset = DecimalParameter(0.95, 0.99, default=0.98, decimals=2, space='buy', optimize=True)
    high_offset = DecimalParameter(0.95, 1.1, default=1.02,  decimals=2, space='sell', optimize=True)
    high_offset_2 = DecimalParameter(0.99, 1.5, default=1.41, decimals=2, space='sell', optimize=True)   

    # Protection
    fast_ewo = 50
    slow_ewo = 200
    ewo_low = IntParameter(-5, 0, default=0, space='buy', optimize=True)
    ewo_high = IntParameter(0, 2, default=0, space='buy', optimize=True)
    ewo_high_safe = IntParameter(2, 8, default=3, space='buy', optimize=True)
    ewo_high_uL = IntParameter(15, 25, default=21, space='buy', optimize=True)
    rsi_buy = IntParameter(55, 70, default=62, space='buy', optimize=True)
    ### BTC 
    bull = DecimalParameter(-0.25, 0.25, default=-0.2, space='buy',decimals=2, optimize=True)
    estop = DecimalParameter(-0.5, 0, default=-0.31, space='sell',decimals=2, optimize=True)
    # dca level optimization
    dca1 = DecimalParameter(low=0.01, high=0.05, decimals=2, default=0.03, space='buy', optimize=True, load=True)
    dca2 = DecimalParameter(low=0.03, high=0.08, decimals=2, default=0.06, space='buy', optimize=True, load=True)
    dca3 = DecimalParameter(low=0.05, high=0.12, decimals=2, default=0.09, space='buy', optimize=True, load=True)

    #trailing stop loss optimiziation
    tsl_target5 = DecimalParameter(low=0.3, high=0.4, default=0.4, decimals=1, space='sell', optimize=True, load=True)
    tsl_target4 = DecimalParameter(low=0.18, high=0.3, default=0.288, space='sell', optimize=True, load=True)
    tsl_target3 = DecimalParameter(low=0.12, high=0.18, default=0.149, space='sell', optimize=True, load=True)
    tsl_target2 = DecimalParameter(low=0.07, high=0.12, default=0.098, space='sell', optimize=True, load=True)
    tsl_target1 = DecimalParameter(low=0.04, high=0.07, default=0.05, space='sell', optimize=True, load=True)
    tsl_target0 = DecimalParameter(low=0.02, high=0.05, default=0.049, space='sell', optimize=True, load=True)
    ts5 = DecimalParameter(low=0.04, high=0.06, default=0.05, space='sell', optimize=True, load=True)
    ts4 = DecimalParameter(low=0.03, high=0.05, default=0.041, space='sell', optimize=True, load=True)
    ts3 = DecimalParameter(low=0.025, high=0.04, default=0.026, space='sell', optimize=True, load=True)
    ts2 = DecimalParameter(low=0.015, high=0.03, default=0.022, space='sell', optimize=True, load=True)
    ts1 = DecimalParameter(low=0.01, high=0.016, default=0.011, space='sell', optimize=True, load=True)
    ts0 = DecimalParameter(low=0.008, high=0.015, default=0.012, space='sell', optimize=True, load=True)

    # Custom ROI Stoploss
    #   These are a tighter set of trailing stoplosses based on time spent in the trade.
    #   If the trade is boring, lets get out of it, 
    #   but lets not just leave profit on the table if it is heading upward at this moment.
    roi_target1 = IntParameter(0, 30, default=28, space="sell", optimize=True, load=True)
    roi_target2 = IntParameter(30, 60, default=34, space="sell", optimize=True, load=True)
    roi_target3 = IntParameter(60, 90, default=60, space="sell", optimize=True, load=True)
    roi1 = DecimalParameter(0.01, 0.02, default=0.013, decimals=3, space="sell", optimize=True, load=True)
    roi2 = DecimalParameter(0.009, 0.015, default=0.015, decimals=3, space="sell", optimize=True, load=True)
    roi3 = DecimalParameter(0.007, 0.01, default=0.01, decimals=3, space="sell", optimize=True, load=True)
    roi_tsl = DecimalParameter(0.001, 0.01, default=0.001, decimals=3, space="sell", optimize=True, load=True)

    unclog_days = IntParameter(1, 7, default=1, space="sell", optimize=True)
    unclog_loss = DecimalParameter(-0.20, -0.01, default=-0.013, decimals=3, space="sell", optimize=True)
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        if current_profit > self.initial_safety_order_trigger:
            logger.debug(f"{trade.pair} - Profit:{current_profit} Trigger:{self.initial_safety_order_trigger}")
            return None
        ...
        if 1 <= count_of_buys <= self.max_safety_orders:
            ...
            if current_profit <= (-1 * abs(safety_order_trigger)):
                try:
                    ...
                    logger.debug(f"Initiating safety order buy #{count_of_buys} for {trade.pair} with stake amount of {stake_amount} which equals {amount}")
                    return stake_amount
                ...
                stake_amount = self.wallets.get_trade_stake_amount(trade.pair, None)
                stake_amount = stake_amount * math.pow(self.safety_order_volume_scale,(count_of_buys - 1))
                logger.debug(f"{trade.pair} Next Safety Order #{count_of_buys} @ Trigger -{safety_order_trigger} Current Profit: {current_profit}")    
                return None
        return None

    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float, current_profit: float, **kwargs):
        # Sell any positions at a loss if they are held for more than unclog_days days.
        if current_profit < self.unclog_loss.value and (current_time - trade.open_date_utc).days >= self.unclog_days.value:
            return 'unclog'
    ...
    def confirm_trade_exit(self, pair: str, trade: Trade, order_type: str, amount: float,
        ...
        logger.info(f'--== EXITING {trade.pair} ==-- Profit {trade.calc_profit_ratio(rate)} Duration {(current_time - trade.open_date_utc).seconds / 60} Minutes Exit Reason {exit_reason}')
    ...
    def custom_stoploss(self, pair: str, trade: 'Trade', current_time: datetime,
        ...
        if (current_profit > self.tsl_target5.value):
            # self.dp.send_msg(f'*** {pair} *** Profit: {current_profit} - lvl5 {self.tsl_target5.value}/{self.ts5.value} activated')
            logger.info(f'*** {pair} *** Profit: {current_profit} - lvl5 {self.tsl_target5.value}/{self.ts5.value} activated')
            return self.ts5.value 
        if (current_profit > self.tsl_target4.value):
            # self.dp.send_msg(f'*** {pair} *** Profit {current_profit} - lvl4 {self.tsl_target4.value}/{self.ts4.value} activated')
            logger.info(f'*** {pair} *** Profit {current_profit} - lvl4 {self.tsl_target4.value}/{self.ts4.value} activated')
            return self.ts4.value 
        ...
        # Custom ROI Stoploss
        # print(f'{timeframe_to_minutes(current_time - trade.open_date_utc)} minutes')
        # print(f'{pair} - {(current_time - trade.open_date_utc).seconds / 60} minutes -- {current_time} - {trade.open_date_utc}')
        trade_time_in_minutes = (current_time - trade.open_date_utc).total_seconds() / 60
        if trade_time_in_minutes >= self.roi_target3.value and current_profit > self.roi3.value:
            logger.info(f"{pair} Profit {current_profit} Minutes {trade_time_in_minutes} - ROI 3 TSL activated")
            # self.dp.send_msg(f'*** {pair} *** Profit {current_profit} - ROI 3 TSL activated')
            return self.roi_tsl.value
        if trade_time_in_minutes >= self.roi_target2.value and current_profit > self.roi2.value:
            logger.info(f"{pair} Profit {current_profit} Minutes {trade_time_in_minutes} - ROI 2 TSL activated")
            # self.dp.send_msg(f'*** {pair} *** Profit {current_profit} - ROI 2 TSL activated')
            return self.roi_tsl.value
        if trade_time_in_minutes >= self.roi_target1.value and current_profit > self.roi1.value:
            logger.info(f"{pair} Profit {current_profit} Minutes {trade_time_in_minutes} - ROI 1 TSL activated")
            # self.dp.send_msg(f'*** {pair} *** Profit {current_profit} - ROI 1 TSL activated')
            return self.roi_tsl.value
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
       ...
       ### Changed this part ###
        ...
        # Check wallet for fees coin balance (BNB for binance)
        self.check_fees_balance()

        return dataframe

    def check_fees_balance(self):
        # Check wallet for fees coin balance (BNB for binance)
        # If balance is less than the set amount, send message to user to top up wallet
        # call this function from the populate_indicators function `self.check_fees_balance()` 
        # Put the following variables in the top of your strategy class
        # fees_balance_notify = True
        # fees_balance_coin = "BNB"
        # fees_balance_notify_amount = 0.015
        # fees_balance_last_checked = None
        if self.dp.runmode.value in ('live') and self.fees_balance_notify and self.wallets:
            # Only check and notify once per candle, not for every pair, per candle.
            if self.fees_balance_last_checked and (datetime.now() - self.fees_balance_last_checked).total_seconds() < 60:
                return
            self.wallets._update_live()
            current_fees_balance = self.wallets.get_free(self.fees_balance_coin)
            if current_fees_balance < self.fees_balance_notify_amount:
                self.dp.send_msg(f"WARNING: You have less than {self.fees_balance_notify_amount} {self.fees_balance_coin} in the wallet. Please top up the wallet to keep receiving your fees discount.", always_send=False)
            logger.info(f"Current {self.fees_balance_coin} balance: {current_fees_balance}")
            self.fees_balance_last_checked = datetime.now()
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[condition1, 'enter_tag'] = 'Safe < EWO2 & RSI Fast'
        ...
        dataframe.loc[condition2, 'enter_tag'] = 'Safe < EWO2'
        ...
        dataframe.loc[condition3, 'enter_tag'] = 'Safe < EWO2 & RSI Fast'
        ...
        dataframe.loc[condition4, 'enter_tag'] = 'Safe < EWO1'
        ...
        dataframe.loc[condition6, 'enter_tag'] = 'HMA SQZ'
        ...
        dataframe.loc[condition8, 'enter_tag'] = 'Safe > EWO2 > High'
        ...
        dataframe.loc[condition9, 'enter_long'] = 1
        dataframe.loc[condition9, 'enter_tag'] = 'EWO2 < low & RSI Fast'
        ...
        dataframe.loc[condition10, 'enter_long'] = 1
        dataframe.loc[condition10, 'enter_tag'] = 'EWO2 < low'
        ...
        dataframe.loc[condition11, 'enter_tag'] = 'Safe > EWO1 > High'
        ...
        dataframe.loc[condition12, 'enter_long'] = 1
        dataframe.loc[condition12, 'enter_tag'] = 'EWO1 < low & RSI Fast'
        ...
        dataframe.loc[condition13, 'enter_tag'] = 'EWO1 < low'
    ...
    def populate_sell_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[condition1, 'exit_tag'] = 'Close > Offset Hi'
        ...
        dataframe.loc[condition2, 'exit_tag'] = 'Close < Offset Lo'
        ...
        dataframe.loc[condition4, 'exit_long'] = 1
        dataframe.loc[condition4, 'exit_tag'] = 'fucking bearzzz'
