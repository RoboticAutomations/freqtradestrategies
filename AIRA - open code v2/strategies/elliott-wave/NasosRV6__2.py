...
buy_params = {'base_nb_candles_buy': 17, 'ewo_high': 3.33, 'ewo_high_2': -0.444, 'ewo_low': -12.788, 'lookback_candles': 1, 'low_offset': 0.985, 'low_offset_2': 0.935, 'profit_threshold': 1.016, 'clucha_rocr_1h': 0.47782, 'clucha_bbdelta_close': 0.02206, 'clucha_bbdelta_tail': 1.02515, 'clucha_close_bblower': 0.03669, 'clucha_closedelta_close': 0.04401, 'base_nb_candles_buy_zema': 25, 'base_nb_candles_buy_zema2': 53, 'low_offset_zema': 0.958, 'low_offset_zema2': 0.961, 'base_nb_candles_buy_ema': 9, 'base_nb_candles_buy_ema2': 75, 'low_offset_ema': 1.067, 'low_offset_ema2': 0.973, 'high_offset_sell_ema': 0.994, 'rsi_buy': 71, 'max_change_pump': 35, 'cofi_adx': 8, 'cofi_ema': 0.639, 'cofi_enabled': False, 'cofi_ewo_high': 5.6, 'cofi_fastd': 40, 'cofi_fastk': 13, 'nfi32_cti_limit': -1.09639, 'nfi32_enabled': True, 'nfi32_rsi_14': 15, 'nfi32_rsi_4': 49, 'nfi32_sma_factor': 0.93391, 'ewo1_enabled': True, 'ewo2_enabled': True, 'ewolow_enabled': True, 'clucha_enabled': True, 'zema_enabled': True, 'vwma_enabled': True, 'base_nb_candles_buy_vwma': 26, 'base_nb_candles_buy_vwma2': 16, 'low_offset_vwma': 0.949, 'low_offset_vwma2': 0.951, 'vwap_enabled': True, 'ewo3_enabled': True, 'nfi24_enabled': True, 'keltner_enabled': True, 'gumbo_enabled': True, 'buy_gumbo_ema': 1.121, 'buy_gumbo_ewo_low': -9.442, 'buy_gumbo_cti': -0.374, 'buy_gumbo_r14': -51.971, 'nfix_39_enabled': True, 'buy_nfix_39_ema': 0.912, 'nfi7_33_enabled': True, 'nfi_38_enabled': True}
...
class NASOSRv6_private_Reinuvader_20211121(IStrategy):
    ...
    base_nb_candles_sell = IntParameter(2, 20, default=sell_params['base_nb_candles_sell'], space='sell', optimize=True)
    ...
    fast_ewo = 50
    slow_ewo = 200
    cooldown_lookback = IntParameter(2, 48, default=protection_params['cooldown_lookback'], space='protection', optimize=True)
    low_profit_optimize = True
    low_profit_lookback = IntParameter(2, 60, default=protection_params['low_profit_lookback'], space='protection', optimize=low_profit_optimize)
    low_profit_stop_duration = IntParameter(12, 200, default=protection_params['low_profit_stop_duration'], space='protection', optimize=low_profit_optimize)
    low_profit_min_req = DecimalParameter(-0.05, 0.05, default=protection_params['low_profit_min_req'], space='protection', decimals=2, optimize=low_profit_optimize)
    ...
    # Zema
    base_nb_candles_buy_zema = IntParameter(5, 80, default=buy_params['base_nb_candles_buy_zema'], space='buy', optimize=True)
    low_offset_zema = DecimalParameter(0.9, 0.99, default=buy_params['low_offset_zema'], space='buy', optimize=True)
    base_nb_candles_buy_zema2 = IntParameter(5, 80, default=buy_params['base_nb_candles_buy_zema2'], space='buy', optimize=True)
    low_offset_zema2 = DecimalParameter(0.9, 0.99, default=buy_params['low_offset_zema2'], space='buy', optimize=True)
    high_offset_sell_ema = DecimalParameter(0.99, 1.1, default=buy_params['high_offset_sell_ema'], space='buy', optimize=True)
    base_nb_candles_buy_ema = IntParameter(5, 80, default=buy_params['base_nb_candles_buy_ema'], space='buy', optimize=True)
    low_offset_ema = DecimalParameter(0.9, 1.1, default=buy_params['low_offset_ema'], space='buy', optimize=True)
    base_nb_candles_buy_ema2 = IntParameter(5, 80, default=buy_params['base_nb_candles_buy_ema2'], space='buy', optimize=True)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[bool(self.ewo1_enabled.value) & (dataframe['rsi_fast'] < 35) & (dataframe['rsi_36'] < 0.45) & (dataframe['close'] < dataframe[f'ma_buy_{self.base_nb_candles_buy.value}'] * self.low_offset.value) & (dataframe['EWO'] > self.ewo_high.value) & (dataframe['rsi'] < self.rsi_buy.value) & (dataframe['volume'] > 0) & (dataframe['close'] < dataframe[f'ma_sell_{self.base_nb_candles_sell.value}'] * self.high_offset.value), ['enter_long', 'enter_tag']] = (1, 'ewo1')
        dataframe.loc[bool(self.ewo2_enabled.value) & (dataframe['rsi_fast'] < 35) & (dataframe['close'] < dataframe[f'ma_buy_{self.base_nb_candles_buy.value}'] * self.low_offset_2.value) & (dataframe['EWO'] > self.ewo_high_2.value) & (dataframe['rsi'] < self.rsi_buy.value) & (dataframe['volume'] > 0) & (dataframe['close'] < dataframe[f'ma_sell_{self.base_nb_candles_sell.value}'] * self.high_offset.value) & (dataframe['rsi'] < 25), ['enter_long', 'enter_tag']] = (1, 'ewo2')
        dataframe.loc[bool(self.ewolow_enabled.value) & (dataframe['rsi_fast'] < 35) & (dataframe['close'] < dataframe[f'ma_buy_{self.base_nb_candles_buy.value}'] * self.low_offset.value) & (dataframe['EWO'] < self.ewo_low.value) & (dataframe['volume'] > 0) & (dataframe['close'] < dataframe[f'ma_sell_{self.base_nb_candles_sell.value}'] * self.high_offset.value), ['enter_long', 'enter_tag']] = (1, 'ewolow')
        ...
        dataframe.loc[nfi_32, ['enter_long', 'enter_tag']] = (1, 'nfi32')
        # vwma
        ...
        buy_offset_vwma = bool(self.vwma_enabled.value) & ((dataframe['close'] < dataframe['vwma_offset_buy']) & (dataframe['pm'] <= dataframe['pmax_thresh']) | (dataframe['close'] < dataframe['vwma_offset_buy2']) & (dataframe['pm'] > dataframe['pmax_thresh']))
        dataframe.loc[buy_offset_vwma, ['enter_long', 'enter_tag']] = (1, 'vwma')
        # vwap
        buy_offset_vwap = bool(self.vwap_enabled.value) & (dataframe['close'] < dataframe['vwap_low']) & (dataframe['tcp_percent_4'] > 0.04) & (dataframe['cti'] < -0.8) & (dataframe['rsi'] < 35) & (dataframe['rsi_84'] < 60) & (dataframe['rsi_112'] < 60) & (dataframe['volume'] > 0)
        dataframe.loc[buy_offset_vwap, ['enter_long', 'enter_tag']] = (1, 'vwap')
        # ewo3
        ewo3 = bool(self.ewo3_enabled.value) & qtpylib.crossed_above(dataframe['rsi_fast'], dataframe['rsi_slow']) & (dataframe['open'] < dataframe['lower']) & (dataframe['tema'] < dataframe['mid']) & (dataframe['cci'] < -100) & ((dataframe['EWO'] < -19.993) | (dataframe['EWO'] > 5.638)) & (dataframe['volume'] > 0)
        dataframe.loc[ewo3, ['enter_long', 'enter_tag']] = (1, 'ewo3')
        # nfi24
        nfi24 = bool(self.nfi24_enabled.value) & (dataframe['ema_12_1h'].shift(12) < dataframe['ema_35_1h'].shift(12)) & (dataframe['ema_12_1h'] > dataframe['ema_35_1h']) & (dataframe['cmf_1h'].shift(12) < 0) & (dataframe['cmf_1h'] > 0) & (dataframe['rsi_14'] < self.buy_24_rsi_14_max) & (dataframe['rsi_14_1h'] > self.buy_24_rsi_14_1h_min)
        dataframe.loc[nfi24, ['enter_long', 'enter_tag']] = (1, 'nfi24')
        # keltner
        keltner = bool(self.keltner_enabled.value) & qtpylib.crossed_above(dataframe['close'], dataframe['kc_upperband']) & (dataframe['rsi'] > dataframe['hline'])
        dataframe.loc[keltner, ['enter_long', 'enter_tag']] = (1, 'keltner')
        # gumbo
        gumbo = bool(self.gumbo_enabled.value) & (dataframe['EWO'] < self.buy_gumbo_ewo_low.value) & (dataframe['bb_middleband2_1h'] >= dataframe['T3_1h']) & (dataframe['T3'] <= dataframe['ema_8'] * self.buy_gumbo_ema.value) & (dataframe['cti'] < self.buy_gumbo_cti.value) & (dataframe['r_14'] < self.buy_gumbo_r14.value)
        dataframe.loc[gumbo, ['enter_long', 'enter_tag']] = (1, 'gumbo')
        # nfix_39
        nfix_39 = bool(self.nfix_39_enabled.value) & (dataframe['ema_200_1h'] > dataframe['ema_200_1h'].shift(12)) & (dataframe['ema_200_1h'].shift(12) > dataframe['ema_200_1h'].shift(24)) & dataframe['bb_lowerband2_40'].shift().gt(0) & dataframe['bb_delta_cluc'].gt(dataframe['close'] * 0.056) & dataframe['closedelta'].gt(dataframe['close'] * 0.01) & dataframe['tail'].lt(dataframe['bb_delta_cluc'] * 0.5) & dataframe['close'].lt(dataframe['bb_lowerband2_40'].shift()) & dataframe['close'].le(dataframe['close'].shift()) & (dataframe['close'] > dataframe['ema_13'] * self.buy_nfix_39_ema.value)
        dataframe.loc[nfix_39, ['enter_long', 'enter_tag']] = (1, 'nfix_39')
        # nfi7_33
        nfi7_33 = bool(self.nfi7_33_enabled.value) & dataframe['moderi_96'] & (dataframe['cti'] < -0.88) & (dataframe['close'] < dataframe['ema_13'] * 0.988) & (dataframe['EWO'] > 6.4) & (dataframe['rsi'] < 32.0) & (dataframe['volume'] < dataframe['volume_mean_4'] * 2.0)
        dataframe.loc[nfi7_33, ['enter_long', 'enter_tag']] = (1, 'nfi7_33')
        # nfi_38
        nfi_38 = bool(self.nfi_38_enabled.value) & (dataframe['pm'] > dataframe['pmax_thresh']) & (dataframe['close'] < dataframe['sma_75'] * 0.98) & (dataframe['EWO'] < -4.4) & (dataframe['cti'] < -0.95) & (dataframe['r_14'] < -97) & (dataframe['crsi_1h'] > 0.5)
        dataframe.loc[nfi_38, ['enter_long', 'enter_tag']] = (1, 'nfi_38')
        return dataframe
