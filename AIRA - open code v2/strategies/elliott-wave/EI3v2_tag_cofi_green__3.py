...
class EI3v2_tag_cofi_green_2(IStrategy):
    ...
    def populate_sell_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            ...
            ['exit_long', 'exit_tag']] = (1, 'Close > Offset Hi 2')
        ...
        dataframe.loc[
            ...
            ['exit_long', 'exit_tag']] = (1, 'Close > Offset Hi 1')
    ...
    def custom_entry_price(self, pair: str, trade: Optional['Trade'], current_time: datetime, proposed_rate: float,
        ...
        new_entryprice = (dataframe['close'].iat[-1] + dataframe['open'].iat[-1]) / 2

        if new_entryprice > proposed_rate:
            entryprice = proposed_rate
            logger.info(f"{trade.pair} using proposed_rate {proposed_rate}")
            self.dp.send_msg(f"{trade.pair} using proposed_rate {proposed_rate}")
            return entryprice
        else:
            entryprice = new_entryprice
            logger.info(f"{trade.pair} using proposed_rate {new_entryprice}")
            self.dp.send_msg(f"{trade.pair} using proposed_rate {new_entryprice}")
            return entryprice
