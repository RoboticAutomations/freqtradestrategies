...
class EI4_t4c0s_V2(IStrategy):
    ...
    def custom_stoploss(self, pair: str, trade: 'Trade', current_time: datetime,
        ...
        if current_candle['max_l'] > .0025: #ignore stoploss if setting new highs
            if SLT2 is not None and current_profit > SLT2:
                self.dp.send_msg(f'*** {pair} *** Profit {display_profit:.4f}% - {slt2:.4f}/{sl2:.4f} activated')
                logger.info(f'*** {pair} *** Profit {display_profit:.4f}% - {slt2:.4f}/{sl2:.4f} activated')
                return SL2
            if SLT1 is not None and current_profit > SLT1:
                self.dp.send_msg(f'*** {pair} *** Profit {display_profit:.4f} - {SLT1:.4f}/{SL1:.4f} activated')
                logger.info(f'*** {pair} *** Profit {display_profit:.4f}% - {slt1:.4f}/{sl1:.4f} activated')
                return SL1

        else:
            if SLT1 is not None and current_profit > SLT1:
                self.dp.send_msg(f'*** {pair} *** Profit {display_profit:.4f}% SWINGING FOR THE MOON!!!')
                logger.info(f'*** {pair} *** Profit {display_profit:.4f}% SWINGING FOR THE MOON!!!')
                return 0.99
