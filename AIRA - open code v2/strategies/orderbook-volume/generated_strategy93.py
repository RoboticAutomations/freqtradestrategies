...
#command = freqtrade backtesting --strategy generated_strategy93_v1 --config backtest_configs/futures_binance.json --timerange 20230101- --timeframe 1h --max-open-trades 3 --cache none --dry-run-wallet 1000

def lerp(a: float, b: float, t: float) -> float:
    return (1 - t) * a + t * b

class generated_strategy93_v1(IStrategy):    
    ...
    profit_ratio_needed = DecimalParameter(-0.100, 0.200, decimals=3, default=-0.188, space="sell", optimize=True)
    exit_only_profit = BooleanParameter(default=True, space="sell", optimize=True)
    max_hold_hours = CategoricalParameter([8, 16, 24, 48, 72, 96, 120, 150, 200, 250, 300, 350, 400, 450, 500, 750, 1000, 1250, 1500], default=450, space="sell", optimize=True)
    ...
    my_custom_stoploss = DecimalParameter(-0.9, -0.1, decimals=1, default=-0.47, space="sell", optimize=True)
    ...
    partial_profit_pct_1 = DecimalParameter(0.005, 0.50, default=0.269, space="sell", optimize=True, decimals=3)
    partial_profit_pct_2 = DecimalParameter(0.01, 1.00, default=0.512, space="sell", optimize=True, decimals=3)
    partial_profit_pct_3 = DecimalParameter(0.02, 1.50, default=1.088, space="sell", optimize=True, decimals=3)
    partial_profit_pct_4 = DecimalParameter(0.05, 2.00, default=0.703, space="sell", optimize=True, decimals=3)
    ...
    max_daily_loss_pct = DecimalParameter(0.01, 0.10, default=0.032, space="buy", optimize=True, decimals=3)
    
    entry_mode = CategoricalParameter(["none", "entry_scaling", "trailing_buy"], default="none", space="buy", optimize=True)
    
    entry_scaling_orders = CategoricalParameter([2, 3, 4, 5], default=5, space="buy", optimize=True)
    entry_scaling_mode = CategoricalParameter(["equal", "increasing", "decreasing"], default="increasing", space="buy", optimize=True)
    
    trailing_buy_pct = DecimalParameter(0.001, 0.10, default=0.009, space="buy", optimize=True, decimals=3)
    trailing_buy_max_distance = DecimalParameter(0.01, 0.20, default=0.072, space="buy", optimize=True, decimals=3)
    trailing_buy_max_hours = CategoricalParameter([1, 2, 4, 8, 12, 24, 48], default=48, space="buy", optimize=True)
    ...
    overbuy_factor = CategoricalParameter([0.8, 0.9, 0.995, 1.0], default=0.6, space="buy", optimize=True)
    
    use_volatility_stake = CategoricalParameter([True, False], default=True, space="buy", optimize=True)
    volatility_size = CategoricalParameter([4,6,8,10,12,14,16,20, 25, 30, 35, 40], default=6, space="buy", optimize=True)
    ...
    walcl_ema_size = CategoricalParameter([14,42,72,84,126,168,212,256,300,400,500], default=72, space="buy", optimize=True)
    ...
    volatility_max_filter_stake = DecimalParameter(0.01, 0.30, default=0.033, space="buy", optimize=True, decimals=3)
    
    use_volume_stake = CategoricalParameter([True, False], default=True, space="buy", optimize=True)
    volume_stake_min_threshold = CategoricalParameter([10_000, 50_000, 100_000, 200_000, 300_000, 400_000, 500_000, 600_000, 700_000, 800_000, 1_000_000, 1_500_000], default=50000, space="buy", optimize=True)
    volume_stake_max_threshold = CategoricalParameter([25_000, 50_000, 75_000, 100_000, 250_000, 500_000, 750_000, 1_000_000, 1_500_000, 2_000_000, 3_000_000, 5_000_000, 7_500_000, 10_000_000], default=10000000, space="buy", optimize=True)
    volume_stake_min_ratio = DecimalParameter(0.5, 1.0, default=0.8, space="buy", optimize=True, decimals=2)
    volume_stake_max_ratio = DecimalParameter(0.5, 1.0, default=0.51, space="buy", optimize=True, decimals=2)
    
    use_drawdown_sizing = CategoricalParameter([True, False], default=False, space="buy", optimize=True)
    drawdown_threshold = DecimalParameter(0.05, 0.30, default=0.1, space="buy", optimize=True, decimals=2)
    drawdown_reduction = DecimalParameter(0.3, 0.8, default=0.38, space="buy", optimize=True, decimals=2)
    drawdown_lookback = CategoricalParameter([10, 20, 30, 50], default=50, space="buy", optimize=True)
    
    use_portfolio_rebalancing = CategoricalParameter([True, False], default=True, space="buy", optimize=True)
    rebalance_threshold = DecimalParameter(0.20, 0.50, default=0.43, space="buy", optimize=True, decimals=2)
    max_position_pct = DecimalParameter(0.15, 0.40, default=0.21, space="buy", optimize=True, decimals=2)
    ...
    tradable_balance_ratio = CategoricalParameter([0.25, 0.35, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.1], default=0.35, space="buy", optimize=True)
    ...
    initial_safety_order_trigger = DecimalParameter(-0.5, 0.0, decimals=3, default=-0.147, space="buy", optimize=True)
    ...
    safety_order_step_scale = IntParameter(1, 20, default=10, space="buy", optimize=True)
    ...
    time_based_dca_candles = CategoricalParameter([1, 2, 4, 8, 12, 24, 48], default=1, space="buy", optimize=True)
    ...
    max_so_multiplier_orig = IntParameter(1, 10, default=3, space="buy", optimize=True)
    ...
    safety_order_volume_scale = DecimalParameter(0.5, 9.0, decimals=1, default=1.0, space="buy", optimize=True)
    ...
    leverage_value = CategoricalParameter([1, 2, 3], default=1, space="buy", optimize=True)

    use_min_volume_filter = CategoricalParameter([True, False], default=True, space="buy", optimize=True)
    min_volume_filter = CategoricalParameter([5_000, 10_000, 20_000, 50_000, 75_000, 100_000, 150_000, 200_000, 300_000, 400_000, 500_000, 750_000, 1_000_000], default=500000, space="buy", optimize=True)
    volume_rolling = CategoricalParameter([24, 48, 72, 96], default=48, space="buy", optimize=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe[f"trend_VWAP5_9_1.15_4.0_low"], dataframe[f"trend_VWAP5_9_1.15_4.0_close"], dataframe[f"trend_VWAP5_9_1.15_4.0_high"] = VWAPB(dataframe, int(36), 1.15)
        # REPULSE Detection
        high_rolling = dataframe['high'].rolling(50).max()
        low_rolling = dataframe['low'].rolling(50).min()
        repulse_up = ((dataframe['low'] <= low_rolling * (1 + 0.005)) & (dataframe['close'] > dataframe['open']) & (dataframe['close'] > low_rolling * (1 + 0.01)))
        repulse_down = ((dataframe['high'] >= high_rolling * (1 - 0.005)) & (dataframe['close'] < dataframe['open']) & (dataframe['close'] < high_rolling * (1 - 0.01)))
        dataframe['over_REPULSE_50_0.5_14'] = (repulse_up.astype(int) - repulse_down.astype(int))
        body_size = abs(dataframe['close'] - dataframe['open']) / dataframe['close']
        volume_ratio = dataframe['volume'] / dataframe['volume'].rolling(50).mean()
        dataframe['over_REPULSE_50_0.5_14_strength'] = abs(dataframe['over_REPULSE_50_0.5_14']) * body_size * volume_ratio
        dataframe['over_REPULSE_50_0.5_14_ma'] = dataframe['over_REPULSE_50_0.5_14_strength'].rolling(14).mean()
        dataframe['over_REPULSE_50_0.5_14_zone'] = dataframe['over_REPULSE_50_0.5_14'].rolling(14).sum()
        return dataframe

    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = []
        conditions.append((dataframe['trend_VWAP5_9_1.15_4.0_close'] < dataframe['high']))
        conditions.append((dataframe['over_REPULSE_50_0.5_14'].rolling(5).min() >= 0))
        ...
        if conditions:
            ...
            dataframe['enter_long'] = dataframe['enter_long'].rolling(window=2).min()
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = []
        conditions.append((dataframe['trend_VWAP5_9_1.15_4.0_close'].shift(1) >= dataframe['low'].shift(1)) & ((dataframe['trend_VWAP5_9_1.15_4.0_close'].shift(1) <= dataframe['close'].shift(1)) & (dataframe['trend_VWAP5_9_1.15_4.0_close'].shift(1) <= dataframe['open'].shift(1)) ) & (dataframe['trend_VWAP5_9_1.15_4.0_close'].shift(2) <= dataframe['low'].shift(2)) & (dataframe['trend_VWAP5_9_1.15_4.0_close'] >= dataframe['low']))
        if conditions:
...
strategy_generator_json = {
    ...
    "params": {
        ...
        "buy": {
            ...
            "cumulative_entry_call": 2,
            "drawdown_lookback": 50,
            "drawdown_reduction": 0.38,
            "drawdown_threshold": 0.1,
            "entry_mode": "none",
            "entry_scaling_mode": "increasing",
            "entry_scaling_orders": 5,
            "exit_conditions_type": "AND",
            "initial_safety_order_trigger": -0.147,
            "leverage_value": 1,
            "max_daily_loss_pct": 0.032,
            "max_position_pct": 0.21,
            "max_so_multiplier_orig": 3,
            "min_volume_filter": 500000,
            "need_cumulative_entry_call": true,
            "over_buy_conditions": 9,
            "over_buy_value": "p1",
            "over_indicator": "REPULSE",
            "over_size1": "p9",
            "over_size2": "p1",
            "over_size3": "p5",
            "overbuy_factor": 0.6,
            "partial_fill_compensation_scale": 0.9,
            "rebalance_threshold": 0.43,
            "safety_order_step_scale": 10,
            "safety_order_volume_scale": 1.0,
            "time_based_dca_candles": 1,
            "tradable_balance_ratio": 0.35,
            "trading_time_mode": "always",
            "trailing_buy_max_distance": 0.072,
            "trailing_buy_max_hours": 48,
            "trailing_buy_pct": 0.009,
            "trend_buy_conditions": 7,
            "trend_buy_value": "p7",
            "trend_indicator": "VWAP5",
            "trend_size1": "p9",
            "trend_size2": "p6",
            "trend_size3": "p7",
            ...
            "volatility_buy_conditions": 0,
            "volatility_buy_value": "p7",
            "volatility_indicator": "LossOfInterest",
            "volatility_max_filter_stake": 0.033,
            "volatility_size": 6,
            "volatility_size1": "p11",
            ...
            "volume_stake_max_ratio": 0.51,
            "volume_stake_max_threshold": 10000000,
            "volume_stake_min_ratio": 0.8,
            "volume_stake_min_threshold": 50000,
        ...
        "sell": {
            ...
            "max_hold_hours": 450,
            "my_custom_stoploss": -0.47,
            "over_sell_conditions": 9,
            "over_sell_value": "p1",
            "over_use_sell": false,
            "parabolic_gain_threshold": 0.48,
            "parabolic_lookback": 7,
            "partial_profit_levels": 3,
            "partial_profit_pct_1": 0.269,
            "partial_profit_pct_2": 0.512,
            "partial_profit_pct_3": 1.088,
            "partial_profit_pct_4": 0.703,
            "profit_ratio_needed": -0.188,
            "trend_sell_conditions": 3,
            "trend_sell_value": "p5",
            "trend_use_sell": true,
            "use_parabolic_exit": true,
            "use_partial_profit": false,
            "volatility_sell_conditions": 1,
            "volatility_sell_value": "p9",
            "volatility_use_sell": false
        },
        "protection": {}
    },
    "ft_stratparam_v": 1,
    "export_time": "2025-09-14 10:54:28.708711+00:00"
}
"""
