...
#command = freqtrade backtesting --strategy generated_strategy98_v1 --config backtest_configs/futures_hyperliquid.json --timerange 20240101- --timeframe 1h --max-open-trades 3 --cache none --dry-run-wallet 1000

def lerp(a: float, b: float, t: float) -> float:
    return (1 - t) * a + t * b

class generated_strategy98_v1(IStrategy):    
    ...
    profit_ratio_needed = DecimalParameter(-0.100, 0.200, decimals=3, default=0.108, space="sell", optimize=True)
    exit_only_profit = BooleanParameter(default=False, space="sell", optimize=True)
    max_hold_hours = CategoricalParameter([8, 16, 24, 48, 72, 96, 120, 150, 200, 250, 300, 350, 400, 450, 500, 750, 1000, 1250, 1500], default=300, space="sell", optimize=True)
    ...
    my_custom_stoploss = DecimalParameter(-0.9, -0.1, decimals=1, default=-0.67, space="sell", optimize=True)
    
    use_partial_profit = CategoricalParameter([True, False], default=False, space="sell", optimize=True)
    partial_profit_levels = CategoricalParameter([2, 3, 4], default=4, space="sell", optimize=True)
    partial_profit_pct_1 = DecimalParameter(0.005, 0.50, default=0.242, space="sell", optimize=True, decimals=3)
    partial_profit_pct_2 = DecimalParameter(0.01, 1.00, default=0.052, space="sell", optimize=True, decimals=3)
    partial_profit_pct_3 = DecimalParameter(0.02, 1.50, default=0.265, space="sell", optimize=True, decimals=3)
    partial_profit_pct_4 = DecimalParameter(0.05, 2.00, default=1.398, space="sell", optimize=True, decimals=3)
    ...
    parabolic_gain_threshold = DecimalParameter(0.10, 0.50, default=0.34, space="sell", optimize=True, decimals=2)
    ...
    max_daily_loss_pct = DecimalParameter(0.01, 0.10, default=0.07, space="buy", optimize=True, decimals=3)
    ...
    trailing_buy_pct = DecimalParameter(0.001, 0.10, default=0.08, space="buy", optimize=True, decimals=3)
    trailing_buy_max_distance = DecimalParameter(0.01, 0.20, default=0.184, space="buy", optimize=True, decimals=3)
    ...
    volatility_size = CategoricalParameter([4,6,8,10,12,14,16,20, 25, 30, 35, 40], default=20, space="buy", optimize=True)
    ...
    walcl_stake_mode = CategoricalParameter([1,2,3,4,5,6], default=6, space="buy", optimize=True)
    walcl_adjustment = CategoricalParameter([0.25, 0.5, 0.75], default=0.25, space="buy", optimize=True)
    
    volatility_size_stake = CategoricalParameter([4, 6, 8, 10, 12, 14, 16, 20, 25], default=10, space="buy", optimize=True)
    volatility_max_filter_stake = DecimalParameter(0.01, 0.30, default=0.26, space="buy", optimize=True, decimals=3)
    
    use_volume_stake = CategoricalParameter([True, False], default=True, space="buy", optimize=True)
    volume_stake_min_threshold = CategoricalParameter([10_000, 50_000, 100_000, 200_000, 300_000, 400_000, 500_000, 600_000, 700_000, 800_000, 1_000_000, 1_500_000], default=800000, space="buy", optimize=True)
    volume_stake_max_threshold = CategoricalParameter([25_000, 50_000, 75_000, 100_000, 250_000, 500_000, 750_000, 1_000_000, 1_500_000, 2_000_000, 3_000_000, 5_000_000, 7_500_000, 10_000_000], default=5000000, space="buy", optimize=True)
    volume_stake_min_ratio = DecimalParameter(0.5, 1.0, default=0.68, space="buy", optimize=True, decimals=2)
    volume_stake_max_ratio = DecimalParameter(0.5, 1.0, default=0.54, space="buy", optimize=True, decimals=2)
    
    use_drawdown_sizing = CategoricalParameter([True, False], default=False, space="buy", optimize=True)
    drawdown_threshold = DecimalParameter(0.05, 0.30, default=0.13, space="buy", optimize=True, decimals=2)
    drawdown_reduction = DecimalParameter(0.3, 0.8, default=0.31, space="buy", optimize=True, decimals=2)
    ...
    max_position_pct = DecimalParameter(0.15, 0.40, default=0.22, space="buy", optimize=True, decimals=2)
    ...
    tradable_balance_ratio = CategoricalParameter([0.25, 0.35, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.1], default=0.5, space="buy", optimize=True)
    ...
    initial_safety_order_trigger = DecimalParameter(-0.5, 0.0, decimals=3, default=0.083, space="buy", optimize=True)
    ...
    volume_spike_window = IntParameter(3, 50, default=20, space="buy", optimize=True)
    ...
    safety_order_step_scale = IntParameter(1, 20, default=6, space="buy", optimize=True)
    ...
    time_based_dca_candles = CategoricalParameter([1, 2, 4, 8, 12, 24, 48], default=4, space="buy", optimize=True)
    ...
    min_volume_filter = CategoricalParameter([5_000, 10_000, 20_000, 50_000, 75_000, 100_000, 150_000, 200_000, 300_000, 400_000, 500_000, 750_000, 1_000_000], default=1000000, space="buy", optimize=True)
    volume_rolling = CategoricalParameter([24, 48, 72, 96], default=72, space="buy", optimize=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Loss of Interest Detection
        price_change = dataframe['close'].pct_change(100)
        recent_rally = price_change > 0.1
        price_range = (dataframe['high'].rolling(100).max() - dataframe['low'].rolling(100).min()) / dataframe['close']
        price_stagnation = price_range < 0.1
        volume_ma = dataframe['volume'].rolling(100 * 2).mean()
        volume_recent = dataframe['volume'].rolling(100).mean()
        volume_decline = (volume_recent / volume_ma) < (1 - 0.2)
        dataframe['trend_LossOfInterest_100_10_20'] = (recent_rally.astype(int) * 0.3 + price_stagnation.astype(int) * 0.35 + volume_decline.astype(int) * 0.35)
        dataframe['trend_LossOfInterest_100_10_20_signal'] = (recent_rally & price_stagnation & volume_decline)
        dataframe['trend_LossOfInterest_100_10_20_duration'] = dataframe['trend_LossOfInterest_100_10_20_signal'].rolling(100).sum()
        ap = (dataframe['high'] + dataframe['low'] + dataframe['close']) / 3
        esa = clean_ta.trend.EMAIndicator(close=ap, window=80).ema_indicator()
        d = clean_ta.trend.EMAIndicator(close=abs(ap - esa), window=9).ema_indicator()
        ci = (ap - esa) / (0.015 * d)
        dataframe['over_William_80_9_24'] = clean_ta.trend.EMAIndicator(close=ci, window=24).ema_indicator()
        dataframe['volatility_ATL_1.75_75_8_1'] = dataframe['low'].rolling(int(131)).min()
        dataframe['volatility_ATL_1.75_75_8_2'] = dataframe['low'].rolling(int(14)).min()
        return dataframe

    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = []
        conditions.append((((dataframe['high'] - dataframe['low']) / dataframe['close']).rolling(50).mean() > 0.02))
        conditions.append(((dataframe['over_William_80_9_24'] > 20) & dataframe['over_William_80_9_24'] <= dataframe['over_William_80_9_24'].shift(1)))
        conditions.append(( dataframe['volatility_ATL_1.75_75_8_2'].shift(3) <= dataframe['low'] ))
        if conditions:
            dataframe.loc[
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = []
        conditions.append(((dataframe['trend_LossOfInterest_100_10_20'] > 0.5) & (dataframe['close'] < dataframe['close'].shift(15))))
        conditions.append(( dataframe['over_William_80_9_24'] < -10))
        conditions.append(( dataframe['volatility_ATL_1.75_75_8_1'].shift(40) > dataframe['low'] ))
        if conditions:
...
strategy_generator_json = {
    ...
    "params": {
        ...
        "buy": {
            ...
            "initial_safety_order_trigger": 0.083,
            "leverage_value": 2,
            ...
            "over_buy_conditions": 6,
            "over_buy_value": "p10",
            "over_indicator": "William",
            "over_size1": "p11",
            "over_size2": "p3",
            "over_size3": "p6",
            "overbuy_factor": 0.9,
            ...
            "time_based_dca_candles": 4,
            "tradable_balance_ratio": 0.5,
            "trading_time_mode": "weekdays_only",
            "trailing_buy_max_distance": 0.184,
            "trailing_buy_max_hours": 8,
            "trailing_buy_pct": 0.08,
            "trend_buy_conditions": 8,
            "trend_buy_value": "p8",
            "trend_indicator": "LossOfInterest",
            "trend_size1": "p12",
            "trend_size2": "p9",
            "trend_size3": "p3",
            ...
            "volatility_buy_value": "p2",
            "volatility_indicator": "ATL",
            "volatility_max_filter_stake": 0.26,
            "volatility_size": 20,
            "volatility_size1": "p12",
            "volatility_size2": "p11",
            "volatility_size3": "p4",
            "volatility_size_stake": 10,
            "volatility_threshold": 0.094,
            "volume_rolling": 72,
            "volume_spike_threshold": 1.25,
            "volume_spike_window": 20,
            "volume_stake_max_ratio": 0.54,
            "volume_stake_max_threshold": 5000000,
            "volume_stake_min_ratio": 0.68,
            "volume_stake_min_threshold": 800000,
        ...
        "sell": {
            ...
            "exit_only_profit": false,
            "max_hold_hours": 300,
            "my_custom_stoploss": -0.67,
            "over_sell_conditions": 1,
            "over_sell_value": "p4",
            "over_use_sell": true,
            "parabolic_gain_threshold": 0.34,
            "parabolic_lookback": 15,
            "partial_profit_levels": 4,
            "partial_profit_pct_1": 0.242,
            "partial_profit_pct_2": 0.052,
            "partial_profit_pct_3": 0.265,
            "partial_profit_pct_4": 1.398,
            "profit_ratio_needed": 0.108,
            "trend_sell_conditions": 10,
            "trend_sell_value": "p3",
            "trend_use_sell": true,
            "use_parabolic_exit": true,
            "use_partial_profit": false,
            "volatility_sell_conditions": 10,
    ...
    "ft_stratparam_v": 1,
    "export_time": "2025-09-16 03:38:12.246102+00:00"
}
"""
