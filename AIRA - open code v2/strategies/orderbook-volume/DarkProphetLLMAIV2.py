...
TradeAction = Literal["LONG_ENTER", "SHORT_ENTER", "NEUTRAL", "LONG_EXIT", "SHORT_EXIT"]
...
class TradingRecommendation(BaseModel):
    ...
    summary: str = Field(
        description="Concise justification (MAXIMUM 300 characters) linking indicators to recommended action",
        max_length=300
    )
...
class GPTTraderConfig(BaseModel):
    ...
    max_tokens: Optional[int] = Field(default=1000, ge=1, le=32768, description="Maximum tokens in response")
...
class DarkProphetLLMAIV2(IFreqaiModel):
    """
    ...
    def _build_indicator_context(self, df: DataFrame) -> str:
        ...
        if bull_div == "1":
            lines.append(f"- ⚠️ BULLISH DIVERGENCE DETECTED")
        if bear_div == "1":
            lines.append(f"- ⚠️ BEARISH DIVERGENCE DETECTED")
...
#
        ...
        lines.append("")
        lines.append("## ⚖️ VWAP (Fair Value)")
        ...
        lines.append("")
        lines.append("## 🏔️ MARKET STRUCTURE (Pivots)")
        ...
        if regime_transition == "1":
            lines.append(f"- ⚠️ **REGIME TRANSITION DETECTED** - Market shifting, be cautious!")
    ...
    def _llm_recommendation(
        ...
        def _parse_action_from_text(raw_text: str) -> TradeAction:
            ...
            m = re.search(r"\b(LONG_ENTER|SHORT_ENTER|LONG_EXIT|SHORT_EXIT|NEUTRAL)\b", cleaned_u)
            if m:
                return m.group(1)  # type: ignore[return-value]

            first_line = cleaned_u.split("\n", 1)[0].strip()
            # Common shorthand
            if "NEUTRAL" in first_line or "HOLD" in first_line or "WAIT" in first_line:
                return "NEUTRAL"
        ...
        try:
            if self.gpt_config.llm_provider == "instructor":
                ...
                try:
                    ...
                    logger.warning(
                        "LLM response is not valid JSON for %s (%s). Using text fallback. Raw (first 500): %s",
                        pair,
                        context,
                        str(content).strip()[:500],
                    )
