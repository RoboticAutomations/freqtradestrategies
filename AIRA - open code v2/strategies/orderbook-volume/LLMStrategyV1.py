...
class LLMStrategy(IStrategy):
    ...
    plot_config = {
        ...
        "subplots": {
            ...
            "expert_signals": {
                ...
                #"expert_hold": {"color": "#607D8B", "type": "bar"},
            },
    ...
    def _notify_llm_decision(self, dataframe: DataFrame, metadata: dict) -> None:
        ...
        try:
            ...
            #hold = last_candle.get("expert_hold", 0)
...
def sanitize_telegram_message(text: str, max_length: int = 4096) -> str:
    """
    Sanitize and truncate text for Telegram MarkdownV2 format.
    ...
    Returns:
        Sanitized and truncated text safe for Telegram
    """
    ...
    sanitized = re.sub(f'([{re.escape(escape_chars)}])', r'\\\1', text)

    # Truncate if too long, leaving room for ellipsis
    if len(sanitized) > max_length:
        ellipsis = sanitize_telegram_message("...")
        sanitized = sanitized[:max_length - len(ellipsis)] + ellipsis

    return sanitized


def sanitize_telegram_multiline(lines: list, max_length: int = 4096, separator: str = "\n") -> str:
    """
    Sanitize and combine multiple lines, respecting length limit.
    Drops lines from the end if total exceeds max_length.

    Args:
        lines: List of text lines to combine
        max_length: Maximum total message length
        separator: Line separator (default newline)

    Returns:
        Combined sanitized message within length limit
    """
    sanitized_lines = []
    current_length = 0
    ...
    for line in lines:
        sanitized_line = sanitize_telegram_message(line, max_length=max_length)
        line_length = len(sanitized_line) + (sep_length if sanitized_lines else 0)

        if current_length + line_length > max_length:
            # Add truncation indicator
            truncation_msg = sanitize_telegram_message("... (truncated)")
            if current_length + len(truncation_msg) + sep_length <= max_length:
                sanitized_lines.append(truncation_msg)
            break

        sanitized_lines.append(sanitized_line)
        current_length += line_length

    return separator.join(sanitized_lines)


def chunk_telegram_message(text: str, max_length: int = 4096) -> list:
    """
    Split long messages into multiple chunks for Telegram.
    Each chunk is sanitized and within the length limit.
    ...
    chunks = []
    while sanitized:
        # Try to split at newline or space near max_length
        if len(sanitized) <= max_length:
            chunks.append(sanitized)
            break

        # Find good break point
        chunk = sanitized[:max_length]
        break_point = max(
            chunk.rfind('\n'),
            chunk.rfind(' '),
            chunk.rfind(',')
        ...
        if break_point > max_length * 0.7:  # Only use break point if it's not too early
            chunk = sanitized[:break_point]
            sanitized = sanitized[break_point:].lstrip()
        else:
            chunk = sanitized[:max_length]
