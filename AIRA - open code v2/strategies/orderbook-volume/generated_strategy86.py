...
#command = freqtrade backtesting --strategy generated_strategy86_v1 --config backtest_configs/futures_100_hyperliquid.json --timerange 20240101- --timeframe 1h --max-open-trades 4 --cache none --dry-run-wallet 1000

def lerp(a: float, b: float, t: float) -> float:
    return (1 - t) * a + t * b

class generated_strategy86_v1(IStrategy):    
    ...
    profit_ratio_needed = DecimalParameter(-0.100, 0.200, decimals=3, default=-0.036, space="sell", optimize=True)
    exit_only_profit = BooleanParameter(default=True, space="sell", optimize=True)
    max_hold_hours = CategoricalParameter([8, 16, 24, 48, 72, 96, 120, 150, 200, 250, 300, 350, 400, 450, 500, 750, 1000, 1250, 1500], default=1000, space="sell", optimize=True)
    ...
    my_custom_stoploss = DecimalParameter(-0.9, -0.1, decimals=1, default=-0.66, space="sell", optimize=True)
    ...
    partial_profit_pct_1 = DecimalParameter(0.005, 0.50, default=0.144, space="sell", optimize=True, decimals=3)
    partial_profit_pct_2 = DecimalParameter(0.01, 1.00, default=0.115, space="sell", optimize=True, decimals=3)
    partial_profit_pct_3 = DecimalParameter(0.02, 1.50, default=0.544, space="sell", optimize=True, decimals=3)
    partial_profit_pct_4 = DecimalParameter(0.05, 2.00, default=0.25, space="sell", optimize=True, decimals=3)
    
    use_parabolic_exit = CategoricalParameter([True, False], default=True, space="sell", optimize=True)
    parabolic_lookback = CategoricalParameter([3, 5, 7, 10, 15], default=3, space="sell", optimize=True)
    parabolic_gain_threshold = DecimalParameter(0.10, 0.50, default=0.29, space="sell", optimize=True, decimals=2)
    ...
    max_daily_loss_pct = DecimalParameter(0.01, 0.10, default=0.062, space="buy", optimize=True, decimals=3)
    ...
    trailing_buy_pct = DecimalParameter(0.001, 0.10, default=0.09, space="buy", optimize=True, decimals=3)
    trailing_buy_max_distance = DecimalParameter(0.01, 0.20, default=0.116, space="buy", optimize=True, decimals=3)
    trailing_buy_max_hours = CategoricalParameter([1, 2, 4, 8, 12, 24, 48], default=2, space="buy", optimize=True)
    ...
    volatility_size = CategoricalParameter([4,6,8,10,12,14,16,20, 25, 30, 35, 40], default=12, space="buy", optimize=True)
    ...
    walcl_size = CategoricalParameter([7, 14, 50, 72, 100, 200, 300, 45, 46], default=42, space="buy", optimize=True)
    walcl_ema_size = CategoricalParameter([14,42,72,84,126,168,212,256,300,400,500], default=300, space="buy", optimize=True)
    ...
    volatility_max_filter_stake = DecimalParameter(0.01, 0.30, default=0.264, space="buy", optimize=True, decimals=3)
    ...
    volume_stake_max_threshold = CategoricalParameter([25_000, 50_000, 75_000, 100_000, 250_000, 500_000, 750_000, 1_000_000, 1_500_000, 2_000_000, 3_000_000, 5_000_000, 7_500_000, 10_000_000], default=2000000, space="buy", optimize=True)
    volume_stake_min_ratio = DecimalParameter(0.5, 1.0, default=0.86, space="buy", optimize=True, decimals=2)
    volume_stake_max_ratio = DecimalParameter(0.5, 1.0, default=0.97, space="buy", optimize=True, decimals=2)
    
    use_drawdown_sizing = CategoricalParameter([True, False], default=False, space="buy", optimize=True)
    drawdown_threshold = DecimalParameter(0.05, 0.30, default=0.22, space="buy", optimize=True, decimals=2)
    ...
    rebalance_threshold = DecimalParameter(0.20, 0.50, default=0.47, space="buy", optimize=True, decimals=2)
    max_position_pct = DecimalParameter(0.15, 0.40, default=0.35, space="buy", optimize=True, decimals=2)
    ...
    tradable_balance_ratio = CategoricalParameter([0.25, 0.35, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.1], default=0.25, space="buy", optimize=True)
    ...
    initial_safety_order_trigger = DecimalParameter(-0.5, 0.0, decimals=3, default=0.089, space="buy", optimize=True)
    ...
    volume_spike_threshold = DecimalParameter(0.7, 3.0, decimals=1, default=3.0, space="buy", optimize=True)
    ...
    time_based_dca_candles = CategoricalParameter([1, 2, 4, 8, 12, 24, 48], default=24, space="buy", optimize=True)
    ...
    min_volume_filter = CategoricalParameter([5_000, 10_000, 20_000, 50_000, 75_000, 100_000, 150_000, 200_000, 300_000, 400_000, 500_000, 750_000, 1_000_000], default=300000, space="buy", optimize=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        keltner = clean_ta.volatility.KeltnerChannel(high=dataframe['high'], low=dataframe['low'], close=dataframe['close'], window=1, window_atr=3.5)
        dataframe['trend_KELTNER_2_3.5_0.6_lband'] = keltner.keltner_channel_lband()
        dataframe['trend_KELTNER_2_3.5_0.6_mband'] = keltner.keltner_channel_mband()
        dataframe['trend_KELTNER_2_3.5_0.6_hband'] = keltner.keltner_channel_hband()
        dataframe["over_VWAP_14_1_4.0"] = clean_ta.volume.VolumeWeightedAveragePrice(dataframe["high"], dataframe["low"], dataframe["close"], dataframe["volume"], window=int(56)).volume_weighted_average_price()
        ap = (dataframe['high'] + dataframe['low'] + dataframe['close']) / 3
        esa = clean_ta.trend.EMAIndicator(close=ap, window=80).ema_indicator()
        d = clean_ta.trend.EMAIndicator(close=abs(ap - esa), window=40).ema_indicator()
        ci = (ap - esa) / (0.015 * d)
        dataframe['volatility_William_80_40_3'] = clean_ta.trend.EMAIndicator(close=ci, window=3).ema_indicator()
        return dataframe

    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = []
        conditions.append((dataframe['trend_KELTNER_2_3.5_0.6_mband'] > dataframe['trend_KELTNER_2_3.5_0.6_mband'].shift(9)))
        ...
        conditions.append((atr_pct < 0.068))
        if conditions:
            dataframe.loc[
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = []
        conditions.append((dataframe['over_VWAP_14_1_4.0'].shift(1) <= dataframe['close']))
        if conditions:
...
strategy_generator_json = {
    ...
    "params": {
        ...
        "buy": {
            ...
            "drawdown_threshold": 0.22,
            "entry_mode": "none",
            "entry_scaling_mode": "decreasing",
            "entry_scaling_orders": 2,
            "exit_conditions_type": "OR",
            "initial_safety_order_trigger": 0.089,
            "leverage_value": 3,
            "max_daily_loss_pct": 0.062,
            ...
            "time_based_dca_candles": 24,
            "tradable_balance_ratio": 0.25,
            "trading_time_mode": "weekdays_only",
            "trailing_buy_max_distance": 0.116,
            "trailing_buy_max_hours": 2,
            "trailing_buy_pct": 0.09,
            ...
            "volatility_indicator": "William",
            "volatility_max_filter_stake": 0.264,
            "volatility_size": 12,
            "volatility_size1": "p11",
            "volatility_size2": "p8",
            "volatility_size3": "p1",
            "volatility_size_stake": 25,
            "volatility_threshold": 0.068,
            "volume_rolling": 24,
            "volume_spike_threshold": 3.0,
            "volume_spike_window": 2,
            "volume_stake_max_ratio": 0.97,
            "volume_stake_max_threshold": 2000000,
            "volume_stake_min_ratio": 0.86,
        ...
        "sell": {
            ...
            "max_hold_hours": 1000,
            "my_custom_stoploss": -0.66,
            "over_sell_conditions": 9,
            "over_sell_value": "p1",
            "over_use_sell": true,
            "parabolic_gain_threshold": 0.29,
            "parabolic_lookback": 3,
            "partial_profit_levels": 4,
            "partial_profit_pct_1": 0.144,
            "partial_profit_pct_2": 0.115,
            "partial_profit_pct_3": 0.544,
            "partial_profit_pct_4": 0.25,
            "profit_ratio_needed": -0.036,
    ...
    "ft_stratparam_v": 1,
    "export_time": "2025-09-12 00:35:09.130576+00:00"
}
"""
