...
#command = freqtrade backtesting --strategy generated_strategy92_v1 --config backtest_configs/futures_100_hyperliquid.json --timerange 20240101- --timeframe 1h --max-open-trades 3 --cache none --dry-run-wallet 1000

def lerp(a: float, b: float, t: float) -> float:
    return (1 - t) * a + t * b

class generated_strategy92_v1(IStrategy):    
    ...
    profit_ratio_needed = DecimalParameter(-0.100, 0.200, decimals=3, default=0.202, space="sell", optimize=True)
    exit_only_profit = BooleanParameter(default=False, space="sell", optimize=True)
    max_hold_hours = CategoricalParameter([8, 16, 24, 48, 72, 96, 120, 150, 200, 250, 300, 350, 400, 450, 500, 750, 1000, 1250, 1500], default=72, space="sell", optimize=True)
    ...
    my_custom_stoploss = DecimalParameter(-0.9, -0.1, decimals=1, default=-0.41, space="sell", optimize=True)
    ...
    partial_profit_pct_1 = DecimalParameter(0.005, 0.50, default=0.197, space="sell", optimize=True, decimals=3)
    partial_profit_pct_2 = DecimalParameter(0.01, 1.00, default=0.103, space="sell", optimize=True, decimals=3)
    partial_profit_pct_3 = DecimalParameter(0.02, 1.50, default=0.102, space="sell", optimize=True, decimals=3)
    partial_profit_pct_4 = DecimalParameter(0.05, 2.00, default=1.925, space="sell", optimize=True, decimals=3)
    ...
    parabolic_gain_threshold = DecimalParameter(0.10, 0.50, default=0.26, space="sell", optimize=True, decimals=2)
    ...
    max_daily_loss_pct = DecimalParameter(0.01, 0.10, default=0.047, space="buy", optimize=True, decimals=3)
    ...
    trailing_buy_pct = DecimalParameter(0.001, 0.10, default=0.025, space="buy", optimize=True, decimals=3)
    trailing_buy_max_distance = DecimalParameter(0.01, 0.20, default=0.179, space="buy", optimize=True, decimals=3)
    ...
    overbuy_factor = CategoricalParameter([0.8, 0.9, 0.995, 1.0], default=0.8, space="buy", optimize=True)
    ...
    volatility_size_stake = CategoricalParameter([4, 6, 8, 10, 12, 14, 16, 20, 25], default=20, space="buy", optimize=True)
    volatility_max_filter_stake = DecimalParameter(0.01, 0.30, default=0.263, space="buy", optimize=True, decimals=3)
    ...
    volume_stake_max_ratio = DecimalParameter(0.5, 1.0, default=0.95, space="buy", optimize=True, decimals=2)
    ...
    drawdown_reduction = DecimalParameter(0.3, 0.8, default=0.42, space="buy", optimize=True, decimals=2)
    ...
    rebalance_threshold = DecimalParameter(0.20, 0.50, default=0.24, space="buy", optimize=True, decimals=2)
    ...
    initial_safety_order_trigger = DecimalParameter(-0.5, 0.0, decimals=3, default=-0.038, space="buy", optimize=True)
    ...
    volume_spike_threshold = DecimalParameter(0.7, 3.0, decimals=1, default=1.0, space="buy", optimize=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        price_change = dataframe['close'].pct_change(90)
        recent_rally = price_change > 0.1
        price_range = (dataframe['high'].rolling(90).max() - dataframe['low'].rolling(90).min()) / dataframe['close']
        price_stagnation = price_range < 0.25
        volume_ma = dataframe['volume'].rolling(90 * 2).mean()
        volume_recent = dataframe['volume'].rolling(90).mean()
        volume_decline = (volume_recent / volume_ma) < (1 - 0.45)
        dataframe['trend_LossOfInterest_90_25_45'] = (recent_rally.astype(int) * 0.3 + price_stagnation.astype(int) * 0.35 + volume_decline.astype(int) * 0.35)
        dataframe['trend_LossOfInterest_90_25_45_signal'] = (recent_rally & price_stagnation & volume_decline)
        dataframe['trend_LossOfInterest_90_25_45_duration'] = dataframe['trend_LossOfInterest_90_25_45_signal'].rolling(90).sum()
        dataframe['over_CoppockCurve_90_40_100'] = (dataframe['close'].pct_change(40) + dataframe['close'].pct_change(100)).rolling(window=90).mean()
        keltner = clean_ta.volatility.KeltnerChannel(high=dataframe['high'], low=dataframe['low'], close=dataframe['close'], window=24, window_atr=3.5)
        dataframe['volatility_KELTNER_7_3.5_3.5_lband'] = keltner.keltner_channel_lband()
        dataframe['volatility_KELTNER_7_3.5_3.5_mband'] = keltner.keltner_channel_mband()
        dataframe['volatility_KELTNER_7_3.5_3.5_hband'] = keltner.keltner_channel_hband()
        return dataframe

    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = []
        conditions.append((((dataframe['high'] - dataframe['low']) / dataframe['close']).rolling(15).mean() > 0.02))
        conditions.append((dataframe['over_CoppockCurve_90_40_100'].diff(periods=20) > 0))
        conditions.append((dataframe['volatility_KELTNER_7_3.5_3.5_hband'] > dataframe['volatility_KELTNER_7_3.5_3.5_hband'].shift(40)))
        if conditions:
            dataframe.loc[
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = []
        conditions.append((dataframe['over_CoppockCurve_90_40_100'] > dataframe['over_CoppockCurve_90_40_100'].shift(30)))
        if conditions:
