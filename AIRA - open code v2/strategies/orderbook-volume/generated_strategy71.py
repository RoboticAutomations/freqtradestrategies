...
#command = freqtrade backtesting --strategy generated_strategy71_v1 --config backtest_configs/futures_hyperliquid.json --timerange 20240101- --timeframe 1d --max-open-trades 15 --cache none --dry-run-wallet 1000

def lerp(a: float, b: float, t: float) -> float:
    return (1 - t) * a + t * b

class generated_strategy71_v1(IStrategy):    
    ...
    profit_ratio_needed = DecimalParameter(-0.100, 0.200, decimals=3, default=-0.06, space="sell", optimize=True)
    ...
    my_custom_stoploss = DecimalParameter(-0.9, -0.1, decimals=1, default=-0.07, space="sell", optimize=True)
    ...
    partial_profit_pct_1 = DecimalParameter(0.005, 0.50, default=0.023, space="sell", optimize=True, decimals=3)
    partial_profit_pct_2 = DecimalParameter(0.01, 1.00, default=0.164, space="sell", optimize=True, decimals=3)
    partial_profit_pct_3 = DecimalParameter(0.02, 1.50, default=0.133, space="sell", optimize=True, decimals=3)
    partial_profit_pct_4 = DecimalParameter(0.05, 2.00, default=0.808, space="sell", optimize=True, decimals=3)
    ...
    parabolic_gain_threshold = DecimalParameter(0.10, 0.50, default=0.32, space="sell", optimize=True, decimals=2)
    ...
    trailing_buy_max_distance = DecimalParameter(0.01, 0.20, default=0.012, space="buy", optimize=True, decimals=3)
    ...
    walcl_ema_size = CategoricalParameter([14,42,72,84,126,168,212,256,300,400,500], default=500, space="buy", optimize=True)
    ...
    volatility_max_filter_stake = DecimalParameter(0.01, 0.30, default=0.181, space="buy", optimize=True, decimals=3)
    ...
    volume_stake_max_threshold = CategoricalParameter([25_000, 50_000, 75_000, 100_000, 250_000, 500_000, 750_000, 1_000_000, 1_500_000, 2_000_000, 3_000_000, 5_000_000, 7_500_000, 10_000_000], default=3000000, space="buy", optimize=True)
    volume_stake_min_ratio = DecimalParameter(0.5, 1.0, default=0.73, space="buy", optimize=True, decimals=2)
    ...
    rebalance_threshold = DecimalParameter(0.20, 0.50, default=0.27, space="buy", optimize=True, decimals=2)
    max_position_pct = DecimalParameter(0.15, 0.40, default=0.24, space="buy", optimize=True, decimals=2)
    ...
    initial_safety_order_trigger = DecimalParameter(-0.5, 0.0, decimals=3, default=-0.289, space="buy", optimize=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe['trend_AWESOME_1_22_10'] = clean_ta.momentum.AwesomeOscillatorIndicator(high=dataframe['high'], low=dataframe['low'], window1=10, window2=22).awesome_oscillator()
        def calculate_sroc(dataframe, emalen, smooth):
            ema = clean_ta.trend.EMAIndicator(close=dataframe['close'], window=emalen).ema_indicator()
            sroc = clean_ta.momentum.ROCIndicator(close=ema, window=smooth).roc()
            return sroc
        dataframe['volatility_SROC_21_8_35'] = calculate_sroc(dataframe, 8, 35)
        return dataframe

    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = []
        conditions.append(((dataframe['volatility_SROC_21_8_35'] > 1) & (dataframe['volatility_SROC_21_8_35'].shift(1) <= 1)))
        ...
        conditions.append((atr_pct < 0.07))
        if conditions:
            dataframe.loc[
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = []
        conditions.append(((dataframe['trend_AWESOME_1_22_10'] > 0) & (dataframe['trend_AWESOME_1_22_10'].shift(60) <= 0)))
        if conditions:
