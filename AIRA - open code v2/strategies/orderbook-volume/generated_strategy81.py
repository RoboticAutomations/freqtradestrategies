...
#command = freqtrade backtesting --strategy generated_strategy81_v1 --config backtest_configs/futures_100_hyperliquid.json --timerange 20240101- --timeframe 1h --max-open-trades 4 --cache none --dry-run-wallet 1000

def lerp(a: float, b: float, t: float) -> float:
    return (1 - t) * a + t * b

class generated_strategy81_v1(IStrategy):    
    ...
    profit_ratio_needed = DecimalParameter(-0.100, 0.200, decimals=3, default=0.02, space="sell", optimize=True)
    exit_only_profit = BooleanParameter(default=False, space="sell", optimize=True)
    max_hold_hours = CategoricalParameter([8, 16, 24, 48, 72, 96, 120, 150, 200, 250, 300, 350, 400, 450, 500, 750, 1000, 1250, 1500], default=120, space="sell", optimize=True)
    ...
    my_custom_stoploss = DecimalParameter(-0.9, -0.1, decimals=1, default=-0.38, space="sell", optimize=True)
    ...
    partial_profit_pct_1 = DecimalParameter(0.005, 0.50, default=0.188, space="sell", optimize=True, decimals=3)
    partial_profit_pct_2 = DecimalParameter(0.01, 1.00, default=0.283, space="sell", optimize=True, decimals=3)
    partial_profit_pct_3 = DecimalParameter(0.02, 1.50, default=1.059, space="sell", optimize=True, decimals=3)
    partial_profit_pct_4 = DecimalParameter(0.05, 2.00, default=0.999, space="sell", optimize=True, decimals=3)
    ...
    max_daily_loss_pct = DecimalParameter(0.01, 0.10, default=0.077, space="buy", optimize=True, decimals=3)
    ...
    trailing_buy_pct = DecimalParameter(0.001, 0.10, default=0.089, space="buy", optimize=True, decimals=3)
    trailing_buy_max_distance = DecimalParameter(0.01, 0.20, default=0.147, space="buy", optimize=True, decimals=3)
    trailing_buy_max_hours = CategoricalParameter([1, 2, 4, 8, 12, 24, 48], default=12, space="buy", optimize=True)
    ...
    walcl_size = CategoricalParameter([7, 14, 50, 72, 100, 200, 300, 45, 46], default=200, space="buy", optimize=True)
    ...
    walcl_adjustment = CategoricalParameter([0.25, 0.5, 0.75], default=0.5, space="buy", optimize=True)
    
    volatility_size_stake = CategoricalParameter([4, 6, 8, 10, 12, 14, 16, 20, 25], default=10, space="buy", optimize=True)
    volatility_max_filter_stake = DecimalParameter(0.01, 0.30, default=0.294, space="buy", optimize=True, decimals=3)
    
    use_volume_stake = CategoricalParameter([True, False], default=False, space="buy", optimize=True)
    volume_stake_min_threshold = CategoricalParameter([10_000, 50_000, 100_000, 200_000, 300_000, 400_000, 500_000, 600_000, 700_000, 800_000, 1_000_000, 1_500_000], default=300000, space="buy", optimize=True)
    volume_stake_max_threshold = CategoricalParameter([25_000, 50_000, 75_000, 100_000, 250_000, 500_000, 750_000, 1_000_000, 1_500_000, 2_000_000, 3_000_000, 5_000_000, 7_500_000, 10_000_000], default=50000, space="buy", optimize=True)
    volume_stake_min_ratio = DecimalParameter(0.5, 1.0, default=0.66, space="buy", optimize=True, decimals=2)
    volume_stake_max_ratio = DecimalParameter(0.5, 1.0, default=0.77, space="buy", optimize=True, decimals=2)
    ...
    drawdown_reduction = DecimalParameter(0.3, 0.8, default=0.74, space="buy", optimize=True, decimals=2)
    ...
    rebalance_threshold = DecimalParameter(0.20, 0.50, default=0.46, space="buy", optimize=True, decimals=2)
    max_position_pct = DecimalParameter(0.15, 0.40, default=0.39, space="buy", optimize=True, decimals=2)
    ...
    tradable_balance_ratio = CategoricalParameter([0.25, 0.35, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.1], default=0.9, space="buy", optimize=True)
    ...
    initial_safety_order_trigger = DecimalParameter(-0.5, 0.0, decimals=3, default=-0.026, space="buy", optimize=True)
    ...
    volume_spike_window = IntParameter(3, 50, default=10, space="buy", optimize=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['trend_CCI_14_0.02_1'] = clean_ta.trend.CCIIndicator(high=dataframe['high'], low=dataframe['low'], close=dataframe['close'], window=14, constant=0.02).cci()
        dataframe['over_RSI_5_9_0.3'] = clean_ta.momentum.rsi(close=dataframe['close'], window=13)
        dataframe['volatility_EMA_6_3_3.0'] = clean_ta.trend.ema_indicator(close=dataframe['close'], window=54)
        return dataframe

    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = []
        conditions.append((dataframe['trend_CCI_14_0.02_1'] <= dataframe['trend_CCI_14_0.02_1'].rolling(window=60).max()))
        conditions.append((dataframe['over_RSI_5_9_0.3'].diff(periods=15) < 0))
        conditions.append((dataframe['volatility_EMA_6_3_3.0'] < dataframe['volatility_EMA_6_3_3.0'].shift(50)))
        if conditions:
            dataframe.loc[
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = []
        conditions.append((dataframe['trend_CCI_14_0.02_1'] > 100))
        if conditions:
...
strategy_generator_json = {
    ...
    "params": {
        ...
        "buy": {
            ...
            "trailing_buy_max_distance": 0.147,
            "trailing_buy_max_hours": 12,
            "trailing_buy_pct": 0.089,
            "trend_buy_conditions": 5,
            "trend_buy_value": "p9",
            "trend_indicator": "CCI",
            "trend_size1": "p14",
            "trend_size2": "p5",
            ...
            "volatility_buy_value": "p9",
            "volatility_indicator": "EMA",
            "volatility_max_filter_stake": 0.294,
            "volatility_size": 35,
            "volatility_size1": "p6",
            "volatility_size2": "p3",
            "volatility_size3": "p5",
            "volatility_size_stake": 10,
            "volatility_threshold": 0.087,
            "volume_rolling": 24,
            "volume_spike_threshold": 1.75,
            "volume_spike_window": 10,
            "volume_stake_max_ratio": 0.77,
            "volume_stake_max_threshold": 50000,
            "volume_stake_min_ratio": 0.66,
            "volume_stake_min_threshold": 300000,
            "walcl_adjustment": 0.5,
            "walcl_ema_size": 84,
            "walcl_size": 200,
            "walcl_stake_mode": 2
        },
        "sell": {
            ...
            "max_hold_hours": 120,
            "my_custom_stoploss": -0.38,
            "over_sell_conditions": 8,
            "over_sell_value": "p5",
            "over_use_sell": false,
            "parabolic_gain_threshold": 0.17,
            "parabolic_lookback": 10,
            "partial_profit_levels": 2,
            "partial_profit_pct_1": 0.188,
            "partial_profit_pct_2": 0.283,
            "partial_profit_pct_3": 1.059,
            "partial_profit_pct_4": 0.999,
            "profit_ratio_needed": 0.02,
            "trend_sell_conditions": 2,
            "trend_sell_value": "p7",
            "trend_use_sell": true,
            "use_parabolic_exit": false,
            "use_partial_profit": true,
            "volatility_sell_conditions": 3,
            "volatility_sell_value": "p10",
            "volatility_use_sell": false
        },
        "protection": {}
    },
    "ft_stratparam_v": 1,
    "export_time": "2025-09-10 20:22:15.837077+00:00"
}
"""
