...
class MoMy5(IStrategy):
    ...
    mri_threshold_high = DecimalParameter(1.0, 5.0, default=2.0, space="buy")
    mri_threshold_low = DecimalParameter(-5.0, -1.0, default=-1.5, space="sell")
    timeframe = "5m"
    ...
    def calculate_mri(self, dataframe: DataFrame) -> DataFrame:
        ...
        dataframe["shooting_star"] = np.where(
            (dataframe["close"] < dataframe["open"])
            & ((dataframe["high"] - dataframe["low"]) > 2 * (dataframe["high"] - dataframe["close"]))
            & ((dataframe["open"] - dataframe["close"]) / (dataframe["high"] - dataframe["low"]).replace(0,
                                                                                                         np.nan) > 0.6),
            1, 0
        )

        dataframe["mri"] = (dataframe["momentum_change"] * dataframe["hammer"]) - (
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (dataframe["mri"] > self.mri_threshold_high.value) & (
                    dataframe['close'] > dataframe['wma']),  # Precio > WMA = Momentum alcista
        ...
        dataframe.loc[
            (dataframe["mri"] < self.mri_threshold_low.value) & (
                    dataframe['close'] < dataframe['wma']),  # Precio < WMA = Momentum bajista
