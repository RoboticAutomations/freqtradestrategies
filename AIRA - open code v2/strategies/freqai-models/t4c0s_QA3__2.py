...
class t4c0s_QA3(BaseRegressionModel):
    """
    ...
    def fit_live_predictions(self, dk: FreqaiDataKitchen, pair: str) -> None:
        ...
        if self.live:
            ...
            candle_diff = len(self.dd.historic_predictions[pair].index) - \
                (num_candles-30 + self.exchange_candles)
            if candle_diff < 0:
                logger.warning(
        ...
        pred = pred_df_sorted.iloc[-int(frequency):].mean()
        print(pred)

        if not warmed_up:
            dk.data['extra_returns_per_train']['&s-maxima_sort_threshold'] = 0.01
            dk.data['extra_returns_per_train']['&s-minima_sort_threshold'] = -0.01
        else:
            if pred['&s-move_target'] >= 0:
                dk.data['extra_returns_per_train']['&s-maxima_sort_threshold'] = pred['&s-move_target']
            else:
                dk.data['extra_returns_per_train']['&s-maxima_sort_threshold'] = 0.001

            if pred['&s-move_target'] <= 0:   
                dk.data['extra_returns_per_train']['&s-minima_sort_threshold'] = pred['&s-move_target']
            else:
                dk.data['extra_returns_per_train']['&s-minima_sort_threshold'] = -0.001
