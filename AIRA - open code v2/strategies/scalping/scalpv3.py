...
class WTX3(IStrategy):
    ...
    plot_config = {
        "main_plot": {
            "wt1": {
                "color": "green",
                "title": "WaveTrend 1 (WT1)",
            },
            "wt2": {
                "color": "red",
                "title": "WaveTrend 2 (WT2)",
            },
            "fast_money_flow": {
                "color": "blue",
                "title": "Fast Money Flow",
            },
            "slow_money_flow": {
                "color": "yellow",
                "title": "Slow Money Flow",
            },
        },
    }
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # 多头开仓条件
        dataframe.loc[
            (dataframe['cross_above'] == 1) & (dataframe['fast_money_flow'] > dataframe['slow_money_flow']) &
            (dataframe['close'] < dataframe['hband']) & (dataframe['rsi'] < 70) & (dataframe['macd'] > dataframe['macd_signal']),
            'enter_long'] = 1

        # 空头开仓条件
        dataframe.loc[
            (dataframe['cross_below'] == 1) & (dataframe['fast_money_flow'] < dataframe['slow_money_flow']) &
            (dataframe['close'] > dataframe['lband']) & (dataframe['rsi'] > 30) & (dataframe['macd'] < dataframe['macd_signal']),
            'enter_short'] = 1
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
            ...
            # 多头平仓条件
            dataframe.loc[
                (dataframe['cross_below'] == 1) & (dataframe['close'] > dataframe['hband']) &
                (dataframe['rsi'] > 70) & (dataframe['macd'] < dataframe['macd_signal']),
                'exit_long'] = 1

            # 空头平仓条件
            dataframe.loc[
                (dataframe['cross_above'] == 1) & (dataframe['close'] < dataframe['lband']) &
                (dataframe['rsi'] < 30) & (dataframe['macd'] > dataframe['macd_signal']),
                'exit_short'] = 1
