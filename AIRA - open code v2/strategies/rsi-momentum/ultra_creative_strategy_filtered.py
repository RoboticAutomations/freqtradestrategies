...
class UltraDynamicStrategy_filtered(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = '3m'
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["mfi"] = ta.mfi(high=dataframe["high"], low=dataframe["low"], close=dataframe["close"],volume=dataframe["volume"],window= 14)
        dataframe["rsi"] = ta.rsi(close=dataframe["close"], window= 14)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[
            (
                ((dataframe["indicator_0_macd"].diff() > 0) 
                 & (dataframe["close"].diff() < 0).fillna(False)) 
                 & ((dataframe["indicator_0_signal"] > 0).fillna(False))
                 & (dataframe['mfi'] < 35)                                      # added
                 #& (dataframe['rsi'] < 50)                                      # added max accuracy, low trades number
            ),
            'enter_long'] = 1
        return dataframe
