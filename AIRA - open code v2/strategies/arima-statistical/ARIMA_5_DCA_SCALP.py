...
class ARIMA_5_DCA_SCALP(IStrategy):
   ...
   ### Trailing Stop ###
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if (forecast_series - current_close).max() > dataframe['move_mean'].iloc[-1] * current_close:
            direction = 'Up'
            dataframe.loc[dataframe.index[-1], 'decision'] = 1

        # Check if, at any point, forecasted close is below current close and down by 5%
        if (forecast_series - current_close).min() < -dataframe['move_mean'].iloc[-1] * current_close:
            direction = 'Down'
            dataframe.loc[dataframe.index[-1], 'decision'] = -1
