...
'''
EXAMPLE ONLY
This strat uses a combination of the potential profit from both the 15min, and 1h candles, and the accuracy percentage to make entry decisions.
As it can't be back-tested, this strategy should be dry-run.  I don't trust it with real money yet, so nor should you.

- CaffeinatedTech
'''

class ARIMA_MTF_Trusting_Futures(IStrategy):
    INTERFACE_VERSION = 2

    # ROI table - Not using it.
    minimal_roi = {
#         "0": 0.195,
...
#         "210": 0
    ...
    tsl_target3 = DecimalParameter(low=0.10, high=0.15, default=0.10, decimals=2,  space='sell', optimize=True, load=True)
    ts3 = DecimalParameter(low=0.025, high=0.04, default=0.035, decimals=3,  space='sell', optimize=True, load=True)
    tsl_target2 = DecimalParameter(low=0.06, high=0.10, default=0.045, decimals=3, space='sell', optimize=True, load=True)
    ts2 = DecimalParameter(low=0.015, high=0.03, default=0.025, decimals=3, space='sell', optimize=True, load=True)
    tsl_target1 = DecimalParameter(low=0.04, high=0.08, default=0.025, decimals=3, space='sell', optimize=True, load=True)
    ts1 = DecimalParameter(low=0.01, high=0.016, default=0.0125, decimals=3, space='sell', optimize=True, load=True)
    tsl_target0 = DecimalParameter(low=0.03, high=0.06, default=0.0125, decimals=3, space='sell', optimize=True, load=True)
    ts0 = DecimalParameter(low=0.005, high=0.012, default=0.0115, decimals=3, space='sell', optimize=True, load=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Check if it's time to retrain for the current pair
        if current_time - self.last_run_time[pair] >= 900:  # Check if an hour has passed
        ...
        if timeleft <= 900 and timeleft != 0:
        ...
        if future_forecast.iloc[-1] > dataframe['OHLC4'].iloc[-1]:
            ...
            direction = 'Down'
        dataframe['decision'] = future_forecast - dataframe['OHLC4']
       

        logger.info(f"{pair} - Test RMSE: {rmse:.3f} | Accuracy: {accuracy_perc:.2f}% | Potential Profit: {abs(reward):.2f}% | Avg. Profit: {move_mean}% | {self.timeframe} Trend: {direction}")
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        condition1 = (
            ...
            (dataframe['accuracy_perc_1h'] >= 98.0) &
            (dataframe['volume'] > 0)
        )

        dataframe.loc[condition1, 'enter_short'] = 1
        dataframe.loc[condition1, 'enter_tag'] = 'Down Trend Soon'
        ...
        dataframe.loc[condition2, 'enter_short'] = 1
        dataframe.loc[condition2, 'enter_tag'] = 'Big Down Trend 1h'
        ...
        condition4 = (
            (dataframe['reward'] >= 0.10) &
            (dataframe['reward_1h'] >= self.predicted_profit_percentage_1h_big.value) &
            (dataframe['accuracy_perc'] >= 98.0) &
            (dataframe['accuracy_perc_1h'] >= 97.0) &
            (dataframe['volume'] > 0)
        )
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[condition5, 'exit_tag'] = 'Up Trend Soon'
