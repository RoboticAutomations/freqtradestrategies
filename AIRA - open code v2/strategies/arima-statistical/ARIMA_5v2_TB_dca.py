...
class ARIMA_5v2_DCA(IStrategy):
    ...
    tsl_target4 = DecimalParameter(low=0.80, high=1.45, default=0.85, decimals=3,  space='sell', optimize=True, load=True)
    ts4 = DecimalParameter(low=0.32, high=0.8, default=0.45, decimals=2,  space='sell', optimize=True, load=True)
    tsl_target3 = DecimalParameter(low=0.55, high=1, default=0.5, decimals=3,  space='sell', optimize=True, load=True)
    ts3 = DecimalParameter(low=0.25, high=0.35, default=0.3, decimals=3,  space='sell', optimize=True, load=True)
    tsl_target2 = DecimalParameter(low=0.4, high=0.5, default=0.2, decimals=3, space='sell', optimize=True, load=True)
    ts2 = DecimalParameter(low=0.15, high=0.25, default=0.2, decimals=3, space='sell', optimize=True, load=True)
    tsl_target1 = DecimalParameter(low=0.35, high=0.6, default=0.4, decimals=3, space='sell', optimize=True, load=True)
    ts1 = DecimalParameter(low=0.3, high=0.45, default=0.3, decimals=3, space='sell', optimize=True, load=True)
    tsl_target0 = DecimalParameter(low=0.25, high=0.35, default=0.3, decimals=3, space='sell', optimize=True, load=True)
    ts0 = DecimalParameter(low=0.08, high=0.4, default=0.08, decimals=3, space='sell', optimize=True, load=True)

    sell_decr_pos = DecimalParameter(0.01, 0.5, decimals=3, default=0.32, space='sell', optimize=False, load=True)
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        return None
class TrailingBuyStrat2(ARIMA_5v2_DCA):
    INTERFACE_VERSION = 3
    ...
    min_uptrend_trailing_profit = 0.0002
    debug_mode = True
    trailing_buy_max_stop = 0.0008  # stop trailing buy if current_price > starting_price * (1+trailing_buy_max_stop)
    trailing_buy_max_buy = 0.0015  # buy if price between uplimit (=min of serie (current_price * (1 + trailing_buy_offset())) and (start_price * 1+trailing_buy_max_buy))
...
class ARIMA_5v2_TB_dca(TrailingBuyStrat2):
    # DCA options
    position_adjustment_enable = True
    initial_safety_order_trigger = -0.05
    max_safety_orders = 2
    safety_order_step_scale = 1  #SS 0.5
