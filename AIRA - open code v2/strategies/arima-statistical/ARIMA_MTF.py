...
#     }
    ...
    window_4h = IntParameter(10, 30, default=21, space='buy', optimize=True, load=True)
    ...
    def populate_indicators_4h(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        train_4h, test_4h = model_selection.train_test_split(dataframe['OHLC4'], train_size=size)

        # Initialize values for the current pair if not already done
        if pair not in self.last_run_time_4h:
            self.last_run_time[pair] = current_time_4h

            logger.info(f"Initial ARIMA 4h Model Training for {pair}")
            # Fit ARIMA model
            start_time_4h = time.time()
            self.arima_model_4h[pair] = auto_arima(train_4h, start_p=1, start_q=1, start_P=1, start_Q=1,
                                                 max_p=5, max_q=5, max_P=5, max_Q=5, seasonal=False,
                                                 stepwise=True, suppress_warnings=True, D=10, max_D=20,
                                                 error_action='ignore')
            fitting_time = time.time() - start_time
            logger.info(f"{pair} - ARIMA 4h Model fitted in {fitting_time:.2f} seconds")


        # Check if it's time to retrain for the current pair
        if current_time - self.last_run_time[pair] >= 43200:  # Check if an 4 hours has passed
            logger.info(f"Auto Fitting 4h ARIMA Model for {pair}")
            self.last_run_time_4h[pair] = current_time
            ...
            logger.info(f"{pair} - ARIMA 4h Model fitted in {fitting_time_4h:.2f} seconds")

        # Use the previously fitted ARIMA model for forecasting
        if self.arima_model_4h[pair] is not None:
            start_time_4h = time.time()
            future_forecast_4h, conf_int_4h = self.arima_model_4h[pair].predict(n_periods=test_4h.shape[0], return_conf_int=True)
            inference_time_4h = time.time() - start_time_4h

        timeleft_4h = current_time - self.last_run_time_4h[pair]
        if timeleft_4h <= 43200 and timeleft_4h != 0:
            logger.info(f"{pair} - ARIMA 4h Model re-optimized in {timeleft_4h:.2f} seconds")

        # Extract upper and lower confidence intervals
        lower_confidence, upper_confidence = conf_int_4h[:, 0], conf_int_4h[:, 1]
        logger.info(f"{pair} - Inference time: {inference_time_4h:.2f} seconds | " \
             f"Current Price: {dataframe['OHLC4'].iloc[-1]:.7f} | 4h Future Forecast: {future_forecast.iloc[-1]:.7f}")
        ...
        logger.info(f"{pair} - Test RMSE: {rmse_4h:.3f} | Accuracy: {accuracy_perc_4h:.2f}% | Potential Profit: {move_4h}% | Avg. Profit: {move_mean_4h}% | 4h Trend: {direction_4h}")
    ...
    def populate_indicators_1h(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        train_1h, test_1h = model_selection.train_test_split(dataframe['OHLC4'], train_size=size)

        # Initialize values for the current pair if not already done
        if pair not in self.last_run_time_1h:
            self.last_run_time[pair] = current_time_1h

            logger.info(f"Initial ARIMA 1h Model Training for {pair}")
            # Fit ARIMA model
            start_time_1h = time.time()
            self.arima_model[pair] = auto_arima(train_1h, start_p=1, start_q=1, start_P=1, start_Q=1,
                                                 max_p=5, max_q=5, max_P=5, max_Q=5, seasonal=False,
                                                 stepwise=True, suppress_warnings=True, D=10, max_D=20,
                                                 error_action='ignore')
            fitting_time = time.time() - start_time
            logger.info(f"{pair} - ARIMA 1h Model fitted in {fitting_time:.2f} seconds")


        # Check if it's time to retrain for the current pair
        if current_time - self.last_run_time[pair] >= 14400:  # Check if an 4 hours has passed
            logger.info(f"Auto Fitting ARIMA 1h Model for {pair}")
            self.last_run_time_1h[pair] = current_time
            ...
            logger.info(f"{pair} - ARIMA 1h Model fitted in {fitting_time_1h:.2f} seconds")

        # Use the previously fitted ARIMA model for forecasting
        if self.arima_model[pair] is not None:
            start_time_1h = time.time()
            future_forecast_1h, conf_int_1h = self.arima_model[pair].predict(n_periods=test_1h.shape[0], return_conf_int=True)
            inference_time_1h = time.time() - start_time_1h

        timeleft_1h = current_time - self.last_run_time_1h[pair]
        if timeleft_1h <= 14400 and timeleft_1h != 0:
            logger.info(f"{pair} - ARIMA 1h Model re-optimized in {timeleft_1h:.2f} seconds")

        # Extract upper and lower confidence intervals
        lower_confidence, upper_confidence = conf_int_1h[:, 0], conf_int_1h[:, 1]
        logger.info(f"{pair} - Inference time: {inference_time_1h:.2f} seconds | " \
             f"Current Price: {dataframe['OHLC4'].iloc[-1]:.7f} | 1h Future Forecast: {future_forecast.iloc[-1]:.7f}")
        ...
        logger.info(f"{pair} - Test RMSE: {rmse_1h:.3f} | Accuracy: {accuracy_perc_1h:.2f}% | Potential Profit: {move_1h}% | Avg. Profit: {move_mean_1h}% | 1h Trend: {direction_1h}")
