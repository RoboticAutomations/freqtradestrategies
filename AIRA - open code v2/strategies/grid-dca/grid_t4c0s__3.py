...
class grid_t4c0s(IStrategy):
    ...
    max_dca_orders = IntParameter(3, 10, default = 7 ,space='buy', optimize=True, load=True)
    dca_scale = DecimalParameter(1.3, 2, default = 1.5 ,space='buy', optimize=True, load=True)
    down_cf = DecimalParameter(1.3, 2.5, default = 2.0, decimals=1 ,space='buy', optimize=True, load=True)
    shift_cf = DecimalParameter(0.3, 1.0, default = 0.5, decimals=1 ,space='buy', optimize=True, load=True)
    max_dca_multiplier = (1 - pow(dca_scale.value,(max_dca_orders.value + 1)))/(1 - dca_scale.value)
    ...
    # stake_to_wallet_ratio = 0.75
        ...
        self.grid_up_spacing_pct = dataframe['move_mean'].iloc[0] 
        self.grid_down_spacing_pct = dataframe['move_mean'].iloc[0] * self.down_cf.value 
        self.grid_trigger_pct = dataframe['move_mean_x'].iloc[0]
        self.grid_shift_pct = dataframe['move_min'].iloc[0] * self.shift_cf.value


        # calculate grid thresholds
        grid_up_shift = grid * (bot_state * self.grid_shift_pct)
        grid_up = avg_price * (1 + self.grid_up_spacing_pct) + grid_up_shift
        grid_down = grid * (1 - self.grid_down_spacing_pct)
        grid_trigger_up = grid * (1 + self.grid_trigger_pct)
        grid_trigger_down = grid * (1 - self.grid_trigger_pct)
        ...
        Close = dataframe.loc[:, 'OHLC4'].values
        High = dataframe.loc[:, 'high'].values
        Low = dataframe.loc[:, 'low'].values
        ...
        while (row <= last_row):
            ...
            if (bot_state == 0):
                # Sell
                if (close > grid_trigger_up) or (high > grid_trigger_up):
                    new_grid = close
                    new_units = 0
                    new_avg_price = close
                
                # Buy
                if (close <= grid_trigger_down) or (low < grid_trigger_down):
                    new_bot_state = 1
                    Buy_1[row] = 1
                    new_units = 1 / close
            ...
            # DCA        
            if ((bot_state >= 1) and (bot_state <= self.max_dca_orders.value)):
                # Sell
                if (close > grid_up) or (high > grid_up):
                    new_bot_state = 0
                    Sell_1[row] = 1
                    new_units = 0
                    new_avg_price = close
                    new_grid = close
                # Buy    
                if (close <= grid_down) or (low <= grid_down):
                    new_bot_state = bot_state + 1
                    Buy_2[row] = 1
                    new_amount = pow(self.dca_scale.value, (bot_state))
                    new_total_amount = (1 - pow(self.dca_scale.value,(bot_state + 1)))/(1 - self.dca_scale.value)
                    new_units = units + (new_amount / close)
                    new_avg_price = new_total_amount / new_units
                    new_grid = close

            if (bot_state == (self.max_dca_orders.value + 1)):
                # Exit trade if below grid and out of dca orders
        ...
        end_time = time.time()
        logger.info(f"{pair} - Bot-State: {bot_state} | Grid: {grid:.7f}, Avg. Price: {avg_price:.7f}")
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['move_min'] = dataframe['move'].min()
        ...
        dataframe['ROLLING_HIGH'] = dataframe['high'].rolling(120).max()
        dataframe['Enable'] = dataframe['ROLLING_HIGH'] * (1 - dataframe['move_mean'])
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        self.custom_info[trade.pair][self.BOT_STATE] = count_of_buys
        self.custom_info[trade.pair][self.AVG_PRICE] = trade.open_rate
        if(len(dataframe) < 1):
            return None
        ...
        if(self.custom_info[trade.pair][self.DATESTAMP] != last_candle['date']):
            ...
            if(last_candle['buy_2'] == 1):
                ...
                if 0 < count_of_buys <= self.max_dca_orders.value:
                    try:
                        # This returns first order stake size
                        stake_amount = filled_buys[0].cost
                        # This then calculates current safety order size
                        stake_amount = stake_amount * pow(self.dca_scale.value, (last_candle['bot_state'] - 1))
