...
def calculate_murrey_math_levels(df, window_size=64):
    ...
    def calculate_mml(mn, finalH):
        dmml = (finalH - mn) / 8
        mml = [mn - dmml * 2]  # -2/8

        for i in range(1, 13):
            mml.append(mml[i - 1] + dmml)
    ...
    maxmax_H = df['high'].rolling(window=window_size).max() * 1.5
    minmin_L = df['low'].rolling(window=window_size).min() * 1.5
    ...
    mml = calculate_mml(finalL, finalH)
...
class AWATRCT(IStrategy):
    ...
    def custom_stake_amount(self, pair: str, current_time: datetime, current_rate: float,
        ...
        if (current_candle['ema'] - current_candle['ema2']) > 0:
            ...
            self.dp.send_msg("Capital Conservation Mode Enabled")
