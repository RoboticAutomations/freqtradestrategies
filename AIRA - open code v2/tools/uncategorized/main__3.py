...
def load_backtest_data(path_to_freqtrade, exchange, timeframes, timerange, info_pairs, info_pairs_tf):
    ...
    if info_pairs:
        ...
        if info_pairs == "all":
            args = ["freqtrade", "download-data", "-c", f"{path_to_freqtrade}user_data/config_backtest.json", 
                "--exchange", exchange, "--timeframes", info_pairs_tf, "--timerange", f"{timerange}", "-c", f"{path_to_freqtrade}user_data/config_backtest.json"]
        else:
            args = ["freqtrade", "download-data", "-c", f"{path_to_freqtrade}user_data/config_backtest.json", 
...
def load_markets(exchange, futures, blacklist):
    if exchange == "binance":
...
def create_backtest_config(path_to_freqtrade, configFile):
    config_path = f"{path_to_freqtrade}user_data/{configFile}"
...
def read_config(path_to_freqtrade, configFile):
    config_path = f"{path_to_freqtrade}user_data/{configFile}"
    ...
    return {'exchange': config['exchange']['name'], 'blacklist': config['exchange']['pair_blacklist']}

def render_config(markets, path_to_freqtrade, exchange, blacklist, futures=False):
...
if __name__ == '__main__':
    ...
    parser.add_argument("-ip", "--info_pairs", dest="info_pairs", metavar="INFO_PAIRS", default="", 
                        help="Informative Pairs 'all' for all pairs with a different timeframe eg. BTC/USDT ETH/USDT")
    ...
    parser.add_argument("-c", "--config", dest="configFile", default="config.json", 
                        help="The name of your config file, defaults to config.json")
    parser.add_argument("-n", "--num_pairs", dest="numpairs", default="50",
                        help="The number of pairs that you want, defaults to 50")

    args = parser.parse_args()
    NUM_PAIRS = int(args.numpairs)
    ...
    configData = read_config(path_to_freqtrade, configFile)
    blacklist = configData['blacklist']

    if exchange == "":
        exchange = configData['exchange']
    create_backtest_config(path_to_freqtrade, configFile)

    # load all available markets in that exchange
    market_list = load_markets(exchange, futures, blacklist)
    render_config(market_list, path_to_freqtrade, exchange, blacklist, futures)
    ...
    while len(market_list) > 0:
        ...
        render_config(this_cycle_pairs, path_to_freqtrade, exchange, blacklist, futures)
    ...
    render_config(best_pairs, path_to_freqtrade, exchange, blacklist, futures)
    start_backtesting(strategy, timeframes, timerange)
    _ = parse_backtest_json()

    print(f"Finished backtesting strategy `{strategy}` for period `{timerange}` on `{exchange}`, here are the best pairs:\n")
