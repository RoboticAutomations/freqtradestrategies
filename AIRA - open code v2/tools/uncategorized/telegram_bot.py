...
class TelegramBot(TelegramBotBase):
    ...
    def setcommands(self, update, context) -> None:
        command = [
            ...
            BotCommand("checktv", "check desirable coins on tradingview")
        ]
    ...
    def help(self, update, context):
        ...
        helptext += "<b>/checktv</b> - <i>check for desirable coins on tradingview</i>\n"
    ...
    def CheckTV(self,update,context):
        if not self._checkifallowed(context._user_id_and_data[0], update):
            return

        self.StartCheckTV(update,context, True if len(context.args) > 0 and context.args[0] == "debug" else False, False if len(context.args) > 0 and context.args[0] == "noscan" else True)
    ...
    def StartMarketScan(self, update, context, debug: bool = False, scanmarkets: bool = True):
        ...
        if debug == False:
            if scanmarkets:
                update.message.reply_text(
                    f"<i>Scanning TradingView for desirable coins.\nThis should be quick depending on the number of pairs.\nOne moment...</i> \u23F3", parse_mode="HTML"
                )
        ...
        for ex in config:
            for quote in config[ex]["quote_currency"]:
                ...
                if not scanner_df.empty:
                    ...
                    outputmsg += "No desirable coin pairs found."
                update.message.reply_text(f"{outputmsg}", parse_mode="HTML")

    def StartCheckTV(self, update, context, debug: bool = False, scanmarkets: bool = True):
        ...
        if debug == False:
            if scanmarkets:
                update.message.reply_text(
                    f"<i>Checking TradingView for desirable coins.\nThis should be quick depending on the number of pairs.\nOne moment...</i> \u23F3", parse_mode="HTML"
                )
                output = subprocess.getoutput("python3 scanner.py")
            ...
            for quote in config[ex]["quote_currency"]:
                ...
                for k,v in data.items():
                    ...
                    if self.enableleverage == False and (
                        str(v).__contains__("DOWN") or str(v).__contains__("UP") or str(v).__contains__("3S") or str(v).__contains__("3L")
                    ):
                        continue
...
def main():
    ...
    dp.add_handler(CommandHandler("checktv", botconfig.CheckTV, Filters.text))
