...
class Telegram(RPCHandler):
    ...
    def _init(self) -> None:
        ...
        handles = [
            CommandHandler("my_strat", self._my_strategy),
    ...
    async def _my_strategy(self, update: Update, context: CallbackContext) -> None:

        """Handles the /my_strat command to change strategy variables using args."""
        try:
            if not context.args or len(context.args) < 2:
                raise RPCException("Usage: /my_strat <variable_name> <new_value>")

            variable_name = context.args[0]
            new_value = context.args[1]

            strategy = self._rpc._freqtrade.strategy  # Access the strategy

            if hasattr(strategy, variable_name):
                try:
                    # Attempt to convert to float (for numeric variables)
                    new_value_float = float(new_value)
                    setattr(strategy, variable_name, new_value_float)
                    await self._send_msg(f"Variable <b>{variable_name}</b> set to <pre>{new_value}</b>", parse_mode=ParseMode.HTML)
                except ValueError:
                    # If float conversion fails, try boolean or string.
                    if new_value.lower() == 'true':
                        setattr(strategy, variable_name, True)
                        await self._send_msg(f"Variable <b>{variable_name}</b> set to True", parse_mode=ParseMode.HTML)
                    elif new_value.lower() == 'false':
                        setattr(strategy, variable_name, False)
                        await self._send_msg(f"Variable <b>{variable_name}</b> set to False.", parse_mode=ParseMode.HTML)
                    else:
                        setattr(strategy, variable_name, new_value)
                        await self._send_msg(f"Variable <b>{variable_name}</b> set to <b>{new_value}</b>", parse_mode=ParseMode.HTML)

            else:
                await self._send_msg(f"Variable <b>{variable_name}</b> not found in strategy.", parse_mode=ParseMode.HTML)
    ...
    async def _help(self, update: Update, context: CallbackContext) -> None:
        ...
        message = (
            "_Bot Control_\n"
            "------------\n"
            "*/my_strat <variable> <value>:* `Overrides the variables content in this lifecycle.`\n"
