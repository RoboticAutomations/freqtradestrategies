...
#        self.addStochasticRSI(14)
    ...
    def ta_HA(self) -> None:
        ...
        df["HA_low"] = df["low"]
        self.df["HA_low"] = df["HA_low"].fillna(0)  
          
        df["HA_open"] = 0.5 * (df["open"].shift() + df["close"].shift())
        self.df["HA_open"] = df["HA_open"].fillna(0)

        df["HA_high"] = df["high"]
        self.df["HA_high"] = df["HA_high"].fillna(0)

        df["HA_close"] = 0.25 * (df["open"] + df["high"] + df["low"] + df["close"])
        self.df["HA_close"] = df["HA_close"].fillna(0)
    ...
    #     self, interval: int = 50, multiplier: int = 2
    # ) -> None:
...
#        df["hist"] = df["MACDh_8_21_5"].fillna(0)
        ...
        df["fast_ma"] = ta.ema(close=self.df["HA_close"], length=8, talib=self.talib)
        df["slow_ma"] = ta.ema(close=self.df["HA_close"], length=21, talib=self.talib)
    ...
    def ta_MACDLead(self) -> None:
        ...
        df['sema'] = ta.ema(self.df["HA_close"], length=fl, talib=self.talib)
        df['lema'] = ta.ema(self.df["HA_close"], length=sl, talib=self.talib)
        df['sema'] = df['sema'].fillna(0)
        df['lema'] = df['lema'].fillna(0)

        df['i1'] = df['sema'] + ta.ema(self.df["HA_close"] - df['sema'], length=fl, talib=self.talib)
        df['i2'] = df['lema'] + ta.ema(self.df["HA_close"] - df['lema'], length=sl, talib=self.talib)
    ...
    def addRSI(self, period: int) -> None:
        ...
        self.df["rsi" + str(period)] = ta.rsi(close=self.df["HA_close"], length=period, talib=self.talib)
    ...
    def addWaveTrend(self) -> None:
        ...
        df['wave_esa'] = ta.ema(df["HA_close"], length=fl, talib=self.talib)
        df['wave_esa_dif'] = ta.ema(abs(df["HA_close"]-df['wave_esa']), length=fl, talib=self.talib)
        self.df['wave_esa'] = df['wave_esa'].fillna(0)
        self.df['wave_esa_dif'] = df['wave_esa_dif'].fillna(0)
        
        df['wave_ci'] = (df["close"]- df['wave_esa']) / (0.015 * df['wave_esa_dif'])
        self.df['wave_ci'] = df['wave_ci'].fillna(0)

        df['wave_trend_1'] = ta.ema(df['wave_ci'], length=sl, talib=self.talib)
        self.df['wave_trend_1'] = df['wave_trend_1'].fillna(0)

        df['wave_trend_2'] = ta.sma(df['wave_trend_1'], length=4, talib=self.talib)
        self.df['wave_trend_2'] = df['wave_trend_2'].fillna(0)

        df["wave_trend_1_pc"] = round((df["wave_trend_1"].shift() - df["wave_trend_1"]) / abs(df["wave_trend_1"].shift()) * -100, 2)
        self.df["wave_trend_1_pc"] = df["wave_trend_1_pc"].fillna(0)
    ...
    def addCandleH(self) -> None:
        ### CANDLE PRICE RANGE STUDY ###
        ...
        candle_height_pcnt = ((candle_height / df['HA_close']) * 100) 
        
        df['range'] = ta.sma(candle_height_pcnt, length=sl, talib=self.talib)
        self.df['range'] = df['range'].fillna(0)
    ...
    def addSMA(self, period: int, addPC: bool = False) -> None:
        ...
        if len(self.df) < period:
            raise Exception(f"addSMA{period} Data range too small. Dataframe = {len(self.df)} periods")

        self.df["sma" + str(period)] = ta.sma(self.df["HA_close"], length=period, talib=self.talib)
        self.df["sma" + str(period)] = self.df["sma" + str(period)].fillna(0)

        if addPC:
            self.df["sma" + str(period) + "_pc"] = round(self.df["sma" + str(period)].pct_change() * 100, 2)
