...
#        pd.set_option('display.max_columns', None)
    ...
    def addAll(self) -> None:
        ...
        self.addSMA(10, True)
        self.addSMA(25, True)
        if self.total_periods >= 55:
            self.addSMA(50, True)
        if self.total_periods >= 110:
            self.addSMA(100, True)
        if self.total_periods >= 210:
            self.addSMA(200)
        ...
        self.addRSI(14,5,True) # add MA period to add Moving Average and True to add pcnt of change
#        self.addStochasticRSI(14)
#        self.addWilliamsR() # default period is 20, add an integer to change default
        self.addMACD(8,21,5) # add fast, slow, signal to be used
...
# testing
        # add closest time of crossover - first signal, second signal - will add {first}{second}_cotime to dataframe
#        self.addCoTime("sma5","sma10")
#        self.addCoTime("sma10","sma50")
#        self.addCoTime("sma50","sma100")

#        self.ta_MACDLead(12,26,5) # add fast, slow, signal to be used

        self.addWaveTrend(addPC=True) # add n1=x for Channel Length (default=10)
                                      # n2=y for Average Length (default=21)
                                      # addPC=True for percent of change on wave trend 1 (default=False)
    ...
    def enableHACandles(self) -> None:
        """Add Heinkin Ashi candles to the data frame""" 
        df = self.df.copy()
              
        df["HA_open"] = 0.5 * (df["open"].shift() + df["close"].shift())
        df["HA_open"] = df["HA_open"].fillna(0)

        df["HA_close"] = 0.25 * (df["open"] + df["high"] + df["low"] + df["close"])
        df["HA_close"] = df["HA_close"].fillna(0)

        self.df["open"] = df["HA_open"]
        self.df["close"] = df["HA_close"]
    ...
    def ta_MACD(self, fast: int=3, slow: int=10, sig: int=5) -> DataFrame:
...
#        df["hist"] = df["MACDh_8_21_5"].fillna(0)
    ...
    def addMACD(self, fast: int=3, slow: int=10, sig: int=5) -> None:
...
#        self.df['obvsm'] = ta.sma(self.df["obv"], length=ma_period, talib=self.talib)
    ...
    def addRSI(self, period: int, ma_period: int = 0, addPC: bool = False) -> None:
        ...
        src = ta.ohlc4(self.df["open"], self.df["high"], self.df["low"], self.df["close"])
#        src = self.df["close"]

        self.df["rsi" + str(period)] = ta.rsi(close=src, length=period, talib=self.talib)
...
#           )
            self.df["rsima" + str(ma_period)] = ta.sma(
                close=self.df["rsi" + str(period)], volume=self.df["volume"], length=(ma_period), talib=self.talib
            )
    ...
    def addWaveTrend(self, n1: int=3, n2: int=21, addPC: bool = False) -> None:
        """ Adds Wave Trend to the data frame """
       
        df = pd.DataFrame()

        ap = self.df["close"]
        esa = ta.ema(close=ap, length=n1, talib=self.talib)
        d = ta.ema(close=abs(ap - esa), length=n1, talib=self.talib)


        df["wave_ci"] = (ap-esa) / (0.015 * d)
        df["wave_fci"] = ta.sma(close=df["wave_ci"], length=4, talib=self.talib)
        df["wave_t1"] = ta.ema(close=df["wave_ci"], length=n2, talib=self.talib)
        df["wave_t2"] = ta.sma(close=df["wave_t1"], length=4, talib=self.talib)
        # functions for buy sell lock out
        df["sell_override"] = ta.sma(close=self.df['close'], length=3, talib=self.talib)
        df["buy_override"] = ta.sma(close=self.df['low'], length=15, talib=self.talib)


        df["wave_ci"] = df["wave_ci"].fillna(0)
        df["wave_fci"] = df["wave_fci"].fillna(0)
        df["wave_t1"] = df["wave_t1"].fillna(0)
        df["wave_t2"] = df["wave_t2"].fillna(0)
        df["sell_override"] = df["sell_override"].fillna(0)
        df["buy_override"] = df["buy_override"].fillna(0)

        if addPC:
            df["wave_t1_pc"] = round((df["wave_t1"] - df["wave_t1"].shift()) / abs(df["wave_t1"]) * 100, 2)
            df["wave_t1_pc"] = df["wave_t1_pc"].fillna(0)
            df["sell_override_pc"] = round((df["sell_override"] - df["sell_override"].shift()) / abs(df["sell_override"]) * 100, 2)
            df["sell_override_pc"] = df["sell_override_pc"].fillna(0)
            df["buy_override_pc"] = round((df["buy_override"] - df["buy_override"].shift()) / abs(df["buy_override"]) * 100, 2)
            df["buy_override_pc"] = df["buy_override_pc"].fillna(0)
...
#        )
    ...
    def simpleMovingAverage(self, period: int) -> float:
        ...
        if period > self.total_periods or period < 1 or period > 200:
...
#    def addCoTime(self, first, second):

#        gtkey = str(first) + str(second) + "_gt"
#        timekey = str(first) + str(second) + "_cotime"
#        print(gtkey)
#        print(timekey)

#        df = self.df.copy()

#        df[gtkey] = df[first] >= df[second]
#        print(df[gtkey])
#        df[timekey] = np.nan
#        df[timekey] = where(
#            (df[timekey].shift().notna() & df[gtkey] == True),
#            df[timekey].shift(),
#            where(
#                df[gtkey] == True,
#                datetime.now(),
...
#        )
#        print(df[timekey])
#        self.df[timekey] = df[timekey]

#        print(df[first + second + "_cotime"])
