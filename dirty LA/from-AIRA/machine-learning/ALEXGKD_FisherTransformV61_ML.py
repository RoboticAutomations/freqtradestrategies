...
class AlexGKD_FisherTransformV61_ML(IStrategy):
    # Strategy parameters
    timeframe = "1h"
    ...
    can_short = True
    set_leverage = 1.0
    max_dca_multiplier = 1.0  # 1.0 = same as initial, 1.5 = 50% more, 2.0 = double
    ...
    if can_short:
        ...
        stoploss = -0.10  # V1.2: CRITICAL FIX - Tightened from -0.524 (52%!) to -0.15 (15%)
    ...
    def get_ml_adjusted_params(self, dataframe: DataFrame, pair: str) -> dict:
        ...
        try:
            ...
            if pair not in self.ml_optimizers:
                self.ml_optimizers[pair] = MLOptimizer(f"GKD_FisherV5_{pair.replace('/', '_').replace(':', '_')}")
...
# PART 3 - Continuing from Part 2
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        try:
            ...
            EnhancedLogger.log_success("Fisher smoothing complete (FIXED trends)", "✅")
...
# PART 5 (FINAL) - Continuing from Part 4
    ...
    def custom_exit(self, pair: str, trade: "Trade", current_time: "datetime", current_rate: float, current_profit: float, **kwargs):
        ...
        EnhancedLogger.log_parameter("Entry Tag", enter_tag, "🏷️")
    ...
    def confirm_trade_exit(self, pair: str, trade: Trade, order_type: str, amount: float, rate: float, 
        ...
        """Enhanced trade exit confirmation with ML learning and detailed logging - FIXED"""
        ...
        if any(word in exit_reason for word in ["roi", "stop_loss", "stoploss", "ml_stop_loss"]):
        ...
        # FIXED: Always allow stoploss exits (don't block negative profits!)
        if exit_reason in ["stop_loss", "ml_stop_loss"]:
            EnhancedLogger.log_success(f"Stoploss exit approved: {exit_reason}", "✅")
        ...
        # FIXED: Only block small POSITIVE profits, not negative ones
        if current_profit > 0 and current_profit < 0.005:
            EnhancedLogger.log_warning("Profit too low (<0.5%), rejecting exit", "⚠️")
            return False
        
        # FIXED: Don't block single buy ROI exits (removed this condition)
        # if (count_of_buys == 1) & any(word in exit_reason for word in ["roi"]):
    ...
    def custom_stake_amount(self, pair: str, current_time: datetime, current_rate: float, 
        ...
        """Standard stake amount - no ML adjustments"""
        return round(proposed_stake,3)

    def adjust_trade_position(self, trade: Trade, current_time: datetime, current_rate: float, 
                             current_profit: float, min_stake: float, max_stake: float, **kwargs) -> Optional[float]:
        """PYRAMIDING DCA - Only add to winning trades when signal confirms"""
        
        # 1. Only DCA when winning (pyramiding, not averaging down)
        if current_profit <= 0:
            return None
        
        # 2. Check if signal still confirms trade direction
        dataframe, _ = self.dp.get_analyzed_dataframe(trade.pair, self.timeframe)
        if len(dataframe) == 0:
            return None
        
        current_signal = dataframe.iloc[-1]
        
        # For LONG trades: only DCA if still getting long signals
        if not trade.is_short and current_signal.get('enter_short', 0) == 1:
            return None  # Don't pyramid long when short signal appears
        
        # For SHORT trades: only DCA if still getting short signals  
        if trade.is_short and current_signal.get('enter_long', 0) == 1:
            return None  # Don't pyramid short when long signal appears
        
        # 3. Check profit threshold for pyramiding
        filled_buys = trade.select_filled_orders(trade.entry_side)
        count_of_buys = len(filled_buys)
        
        # Minimum profit before first pyramid (e.g., 2%)
        min_pyramid_profit = 0.02
        
        # Progressive profit targets for additional pyramids
        profit_thresholds = [0.02, 0.05, 0.10, 0.15, 0.20]  # 2%, 5%, 10%, 15%, 20%
        
        if count_of_buys >= len(profit_thresholds) + 1:
            return None  # Max pyramids reached
        
        required_profit = profit_thresholds[count_of_buys - 1] if count_of_buys - 1 < len(profit_thresholds) else 0.30
        
        if current_profit < required_profit:
            return None  # Profit not high enough yet
        
        EnhancedLogger.log_section(f"PYRAMID DCA - {trade.pair}", "📈")
        EnhancedLogger.log_parameter("Current Profit", f"{current_profit:.2%}", "💰")
        EnhancedLogger.log_parameter("Pyramid Level", count_of_buys, "🔺")
        EnhancedLogger.log_parameter("Required Profit", f"{required_profit:.2%}", "🎯")
        
        try:
            actual_initial_stake = filled_buys[0].cost
            
            # Calculate pyramid stake (smaller than initial, scaling down)
            # Each pyramid is 50% of previous
            pyramid_stake = actual_initial_stake * (0.5 ** (count_of_buys - 1))
            
            # Cap at max allowed
            max_allowed_stake = actual_initial_stake * self.max_dca_multiplier
            stake_amount = min(pyramid_stake, max_allowed_stake, max_stake)
            
            EnhancedLogger.log_parameter("Pyramid Stake", f"{stake_amount:.4f}", "💵")
            EnhancedLogger.log_success(f"Pyramid order #{count_of_buys + 1} approved", "✅")
            
            return stake_amount
            
        except Exception as e:
            EnhancedLogger.log_error(f"Pyramid calculation failed: {e}", "💥")
            return None
    ...
    def custom_stoploss(self, pair: str, trade: Trade, current_time: datetime, current_rate: float, 
        ...
        """Disabled - using fixed stoploss only"""
        return None  # DISABLED: Let strategy use fixed stoploss = -0.06
    ...
    def leverage(self, pair: str, current_time: datetime, current_rate: float, proposed_leverage: float, 
        ...
        try:
            dataframe, _ = self.dp.get_analyzed_dataframe(pair, self.timeframe)
            base_leverage = self.set_leverage if self.can_short else 1.0
        ...
        default_leverage = self.set_leverage if self.can_short else 1.0
