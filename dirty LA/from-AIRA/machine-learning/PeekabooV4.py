...
class CusumMeanDetector():
    ...
    def _reset(self) -> None:
        self.current_t = 0
        self.current_obs = deque(maxlen=self._t_warmup)
        self.running_sum = 0
        self.running_sum_squares = 0

    def _update_data(self, y: torch.tensor) -> None:
        self.current_t += 1
        self.current_obs.append(y.item())
        self.running_sum += y.item()
        self.running_sum_squares += y.item()**2

    def _init_params(self) -> None:
        self.current_mean = self.running_sum / self._t_warmup
        self.current_std = (self.running_sum_squares / self._t_warmup - self.current_mean**2)**0.5

    def _check_for_changepoint(self) -> Tuple[float,bool]:
        standardized_sum = sum((x - self.current_mean) / self.current_std for x in self.current_obs)
    ...
    def _get_prob(self, y: float) -> bool:
        p = torch.distributions.normal.Normal(0,1).cdf(torch.abs(torch.tensor(y)))
        prob = 2*(1 - p)
        return prob
...
class PeekabooV4(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[condition32, 'enter_long'] = 1
        dataframe.loc[condition32, 'enter_tag'] = 'minima_check_1_delay long'
