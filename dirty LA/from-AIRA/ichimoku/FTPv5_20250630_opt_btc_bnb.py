...
#5_1    Entry: RSI Oversold (Long) / RSI Overbought (Short)
#5_3    SuperDCA al 1/3 stoploss
#5_4    opt para BTC BNB
...
################################################################################################################################
    ...
    def confirm_trade_exit(self, pair: str, trade: Trade, order_type: str, amount: float,
        ...
        # Allow force exit
        if exit_reason == "force_exit" or exit_reason == "KILL OLD TRADE":
            return True

        #exit conditions
        trade_age = current_time - trade.open_date_utc
        profit_ratio = trade.calc_profit_ratio(rate)
        exit_condition1 = (trade_age > timedelta(hours=self.min_hours_cte.value))
        exit_condition2 = (profit_ratio >= self.min_roi_cte.value)
...
################################################################################################################################
    ...
    use_RSI_Over_longs = BooleanParameter(default=False, space="buy", optimize=optimize_buy_signals)
    use_RSI_Over_shorts = BooleanParameter(default=False, space="buy", optimize=optimize_buy_signals)
    use_BBB_longs = BooleanParameter(default=False, space="buy", optimize=optimize_buy_signals)             #PENDIENTE: para salir, la sma del medio (custom exit w / entry tag y cond cierre del otro lado de la media)
    use_BBB_shorts = BooleanParameter(default=False, space="buy", optimize=optimize_buy_signals)

    #buyspace - general
    optimize_buy_general = False
    ...
    use_bollinger_exit_long = BooleanParameter(default=True, space="sell", optimize=optimize_sell_signals)                            #(custom)
    use_bollinger_exit_short = BooleanParameter(default=True, space="sell", optimize=optimize_sell_signals)                           #(custom)
    use_BBB_shorts_exit = BooleanParameter(default=False, space="sell", optimize=optimize_sell_signals)                                
    use_BBB_longs_exit = BooleanParameter(default=False, space="sell", optimize=optimize_sell_signals)

    #sellspace - Kill Old Trades (KOT)
    optimize_KOT = False
    use_kot =  BooleanParameter(default=False, space="sell", optimize=optimize_KOT)
    sell_clear_old_trade = IntParameter(120, 360, default=120, space="sell", optimize=optimize_KOT)             #edad en velas (120 velas son 10 horas)
    ...
    use_BB_csl = BooleanParameter(default=False, space="sell", optimize=optimize_customSL)                               #bollinger + ATR
    csl_atr_mult_BB = DecimalParameter(0.5, 1.0, default=6.0, space="sell", decimals=2, optimize=optimize_customSL)

    #sellspace - confirm trade exit
    Opt_cte = True
    min_hours_cte = IntParameter(2, 10, default=4, space="sell", optimize=Opt_cte)
    min_roi_cte = DecimalParameter(0.01, 0.10, decimals=2, default=minroi, space="sell", optimize=Opt_cte)
...
################################################################################################################################
    ...
    max_entry_position_adjustment = 3       # 2 original
    ...
    step3 = 1*stoploss/3                    #SUPERDCA 1
    step4 = 2*stoploss/3                    #SUPERDCA 2
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        if current_profit > self.step2 and trade.nr_of_successful_entries == 2:  
            return None
        if current_profit > self.step3 and trade.nr_of_successful_entries == 3:         #SUPERDCA 1
            return None
        if current_profit > self.step4 and trade.nr_of_successful_entries == 4:         #SUPERDCA 2
            return None
        ...
        # Obtain pair dataframe                                 #no se usa
        #dataframe, _ = self.dp.get_analyzed_dataframe(trade.pair, self.timeframe)
        #last_candle = dataframe.iloc[-1].squeeze()             #no se usa
        ...
        try:
            ...
            # Coste base de la primera entrada (post o pre‐apalancamiento):
            if self.dca_post_leverage:
                stake = filled_entries[0].cost      
            else:
                stake = filled_entries[0].stake_amount_filled              #se puede usar trade.stake_amount

            already_committed = trade.stake_amount

            if count_of_entries > 1:
                stake = stake * (1 + (count_of_entries - 1) * self.dca_increase)

            additional = stake - already_committed
            avail_funds = self.wallets.get_free(self.stakename)
            total_funds = self.wallets.get_total(self.stakename)

            # 5) Ajuste según fondos libres
            if additional < avail_funds:
                # Cubres todo el incremento
                last_logged = trade.get_custom_data(key='last_dca_log_count', default=None)
                if last_logged is None or last_logged != count_of_entries:
                    msg = (f"🟡 DCA: {current_time} Increasing {trade.pair} by {additional:.2f}{self.stakename} - new total {stake:.2f}{self.stakename} - total dca entries {count_of_entries}"
                        f" Av.: {avail_funds:.2f}/{total_funds:.2f} {self.stakename}")
                    logger.info(msg)
                    self.dp.send_msg(msg)
                    # Record that we logged for this entry count
                    trade.set_custom_data(key='last_dca_log_count', value=count_of_entries)
                return additional, "DCA INCREASING"
            
            elif avail_funds > 1:
                # Sólo puedes cubrir parcialmente
                last_logged = trade.get_custom_data(key='last_dca_log_count', default=None)
                if last_logged is None or last_logged != count_of_entries:
                    msg = (f"🟠 DCA: {current_time} Increasing {trade.pair} by {avail_funds:.2f}{self.stakename} - new total {already_committed+avail_funds:.2f}{self.stakename} - total dca entries {count_of_entries}"
                        f" Av.: {avail_funds:.2f}/{total_funds:.2f} {self.stakename} (INCOMPLETE DCA)")
                    logger.info(msg)
                    self.dp.send_msg(msg)
                    # Record that we logged for this entry count
                    trade.set_custom_data(key='last_dca_log_count', value=count_of_entries)
                return avail_funds, "DCA PARTIAL"
            else:
                # No hay fondos libres
                return None
...
##########################################################################
        ...
        if self.use_atr_exit.value and current_profit > atr_exit_profit and exit_conditions:      #atr exit
            ...
            msg_atrExit = f"❇️ ATR: {current_time} {trade.pair} exit!! Profit {100*current_profit:.2f}% ({current_profit*trade.stake_amount:.2f} {self.stakename}). Total stake was {trade.stake_amount:.2f} {self.stakename} ({count_of_entries} entry(s))."
...
##########################################################################
        #♾️BB CUSTOM EXIT
        min_bb_roi = 0.02    
        if trade.is_short and self.use_bollinger_exit_short.value and current_rate <= last_candle['BB_lower_1h'] and current_profit > min_bb_roi and exit_conditions:
            msg_bbexit = f"♾️ BBL: {current_time} {trade.pair} exit!! Profit {100*current_profit:.2f}% ({current_profit*trade.stake_amount:.2f} {self.stakename}). Total stake was {trade.stake_amount:.2f} {self.stakename} ({count_of_entries} entry(s))."
            self.dp.send_msg(msg_bbexit)
            logger.info(msg_bbexit)            
            return("BOLLINGER SHORT EXIT")

        if not trade.is_short and self.use_bollinger_exit_long.value and current_rate >= last_candle['BB_upper_1h'] and current_profit > min_bb_roi and exit_conditions:
            msg_bbexit = f"♾️ BBS: {current_time} {trade.pair} exit!! Profit {100*current_profit:.2f}% ({current_profit*trade.stake_amount:.2f} {self.stakename}). Total stake was {trade.stake_amount:.2f} {self.stakename} ({count_of_entries} entry(s))."
            self.dp.send_msg(msg_bbexit)
            logger.info(msg_bbexit)       
...
##########################################################################
        ...
        min_mfi_roi = 0.06
        if trade.is_short and self.use_mfi_exit_short.value and last_candle['mfi'] <= 11 and current_profit > min_mfi_roi and exit_conditions:
            msg_mfiexit = f"🚀​ MFI: {current_time} {trade.pair} exit!! Profit {100*current_profit:.2f}% ({current_profit*trade.stake_amount:.2f} {self.stakename}). Total stake was {trade.stake_amount:.2f} {self.stakename} ({count_of_entries} entry(s))."
            self.dp.send_msg(msg_mfiexit)
            logger.info(msg_mfiexit)            
            return("MFI SHORT EXIT")

        if not trade.is_short and self.use_mfi_exit_long.value and last_candle['mfi'] >= 89 and current_profit > min_mfi_roi and exit_conditions:
            msg_mfiexit = f"🚀​ MFI: {current_time} {trade.pair} exit!! Profit {100*current_profit:.2f}% ({current_profit*trade.stake_amount:.2f} {self.stakename}). Total stake was {trade.stake_amount:.2f} {self.stakename} ({count_of_entries} entry(s))."
            self.dp.send_msg(msg_mfiexit)
            logger.info(msg_mfiexit)       
...
##########################################################################
        ...
        if self.use_kot.value:
            ...
            if current_time - timedelta(minutes=int(self.timeframe_minutes * self.sell_clear_old_trade.value)) >= trade.open_date_utc:
                if (profit_ratio >= (-0.01 * self.sell_clear_old_trade_profit.value)):       
                    #si el profit es igual o mayor que el valor mínimo de cierre
                    msgKOT=(f"👻 KOT: {current_time} {trade.pair} {trade.trade_direction} killed at {trade_age}. Profit {profit_ratio*100:.2f}%. Total stake was {trade.stake_amount:.2f} {self.stakename} ({count_of_entries} entry(s)).")
                    logger.info(msgKOT)
...
################################################################################################################################
    ...
    # RSI OVERSOLD/BOUGHT ##########################################################################################################
    # Long cuando rsi cae mucho, short cuando esta muy alto
    # No se usa: el winrate no es suficiente para cubrir las perdidas (wr alrededor de 90%)
    # Requiere mejoras 
        if self.use_RSI_Over_longs.value:
            # Long entry: RSI dump
            df.loc[
                (
                    #(df['btc_trend'].isin(['DOWN', 'NA'])) &                            #Check BTC trend 1H
                    (df['adx_1h'] > 25) &                                                #strong trend
                    (df['mfi'] < 20) &                                                   #mfi dump
                    (df['rsi_1h'] < 30) &                                                #rsi dump
                    (df['close_1h'] > df['sma200_1h'])                                   #over sma200_1h
                ),
                ['enter_long', 'enter_tag']
            ] = (1, 'RSI_Over_Long')
                   
        if self.use_RSI_Over_shorts.value:
            # Short entry: RSI pump
            df.loc[
                (
                    #(df['btc_trend'].isin(['DOWN', 'NA'])) &                            #Check BTC trend 1H
                    (df['adx_1h'] > 25) &                                                #strong trend
                    (df['mfi'] > 80) &                                                   #mfi pump
                    (df['rsi_1h'] > 70) &                                                #rsi pump
                    (df['close_1h'] < df['sma200_1h'])                                   #over sma200_1h
                ),
                ['enter_short', 'enter_tag']
            ] = (1, 'RSI_Over_Short')
    ...
    # BOLLINGER BAND BOUNCE (BBB) ##########################################################################################################

        if self.use_BBB_longs.value:
            # Long entry: RSI dump
            df.loc[
                (
                    (df['btc_trend'].isin(['UP', 'NA'])) &                                #Check BTC trend 1H
                    (df['close'] < df['BB_lower_1h']) &                                     #bollinger
                    (df['rsi_1h'] < 30)                                                     #rsi dump
                    &(df['close_1h'] > df['sma200_1h'])                                   #over sma200_1h
                ),
                ['enter_long', 'enter_tag']
        ...
        if self.use_BBB_shorts.value:
            # Long entry: RSI dump
            df.loc[
                (
                    (df['btc_trend'].isin(['DOWN', 'NA'])) &                                #Check BTC trend 1H
                    (df['close'] > df['BB_upper_1h']) &                                     #bollinger
                    (df['rsi_1h'] > 70)                                                     #rsi pump
                    #&(df['mfi_1h'] > 70)                                                    #mfi pump
                    &(df['close_1h'] < df['sma200_1h'])                                     #over sma200_1h
                ),
                ['enter_short', 'enter_tag']
            ] = (1, 'BBB_Short')
    ...
    def populate_exit_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.use_BBB_longs_exit.value:
            df.loc[
                (
                    (df['close'] > df['BB_upper_1h'])              
                ),
                ['exit_long', 'exit_tag']
            ] = (1, 'BB_ExitLong_OverUpper')

        if self.use_BBB_shorts_exit.value:
            df.loc[
                (
                    (df['close'] < df['BB_lower_1h'])              
                ),
                ['exit_short', 'exit_tag']
            ] = (1, 'BB_ExitShort_UnderLower')
