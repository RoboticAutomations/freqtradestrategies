...
class haFbm(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.dp.runmode.value in ('live', 'dry_run'):
            ha_df['signal_MEAN_UP_0'] = ha_df['signal_UP_0'].rolling(int(harmonics[0])).mean() * self.dc_0.value
            ha_df['signal_MEAN_DN_0'] = ha_df['signal_DN_0'].rolling(int(harmonics[0])).mean() * self.dc_0.value
        else:
            # this step is for hyperopting and backtesting to simulate the rolling window of the exchange in live mode.
            # this needs to be a value specific for YOUR exchange and/or timeframe.
            ha_df['signal_MEAN_UP_0'] = ha_df['signal_UP_0'].rolling(int(harmonics[0])).mean() * self.dc_0.value
        ...
        if self.dp.runmode.value in ('live', 'dry_run'):
            ha_df['signal_MEAN_UP_1'] = ha_df['signal_UP_1'].rolling(int(harmonics[1])).mean() * self.dc_1.value
            ha_df['signal_MEAN_DN_1'] = ha_df['signal_DN_1'].rolling(int(harmonics[1])).mean() * self.dc_1.value
        else:
            # this step is for hyperopting and backtesting to simulate the rolling window of the exchange in live mode.
            # this needs to be a value specific for YOUR exchange and/or timeframe.
            ha_df['signal_MEAN_UP_1'] = ha_df['signal_UP_1'].rolling(int(harmonics[1])).mean() * self.dc_1.value
        ...
        if self.dp.runmode.value in ('live', 'dry_run'):
            ha_df['signal_MEAN_UP_2'] = ha_df['signal_UP_2'].rolling(int(harmonics[2])).mean() * self.dc_2.value
            ha_df['signal_MEAN_DN_2'] = ha_df['signal_DN_2'].rolling(int(harmonics[2])).mean() * self.dc_2.value
        else:
            # this step is for hyperopting and backtesting to simulate the rolling window of the exchange in live mode.
            # this needs to be a value specific for YOUR exchange and/or timeframe.
            ha_df['signal_MEAN_UP_2'] = ha_df['signal_UP_2'].rolling(int(harmonics[2])).mean() * self.dc_2.value
        ...
        ha_df['market_bias0'] = (ha_df['signal_MEAN_UP_0'] / abs(ha_df['signal_MEAN_DN_0']))
        ha_df['market_dir0'] = np.where((ha_df['market_bias0'] > ha_df['market_bias0'].shift()), 1, np.where((ha_df['market_bias0'] < ha_df['market_bias0'].shift()), -1, 0))  
        ha_df['market_dir0'] = np.where((ha_df['market_bias0'] < 1), 0, ha_df['market_dir0'])      
        ha_df['market_bias1'] = (ha_df['signal_MEAN_UP_1'] / abs(ha_df['signal_MEAN_DN_1']))
        ha_df['market_dir1'] = np.where((ha_df['market_bias1'] > ha_df['market_bias1'].shift()), 1, np.where((ha_df['market_bias1'] < ha_df['market_bias1'].shift()), -1, 0))  
        ha_df['market_dir1'] = np.where((ha_df['market_bias1'] < 1), 0, ha_df['market_dir1'])      
        ha_df['market_bias2'] = (ha_df['signal_MEAN_UP_2'] / abs(ha_df['signal_MEAN_DN_2']))
        ha_df['market_dir2'] = np.where((ha_df['market_bias2'] > ha_df['market_bias2'].shift()), 1, np.where((ha_df['market_bias2'] < ha_df['market_bias2'].shift()), -1, 0))  
        ha_df['market_dir2'] = np.where((ha_df['market_bias2'] < 1), 0, ha_df['market_dir2'])      
        ha_df['market_bias_sum'] = ha_df['market_bias'] + ha_df['market_bias0'] + ha_df['market_bias1'] + ha_df['market_bias2']
        ha_df['market_dir_sum'] = ha_df['market_dir'] + ha_df['market_dir0'] + ha_df['market_dir1'] + ha_df['market_dir2']
