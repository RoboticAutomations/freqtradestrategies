...
def _stepped_range(df: pd.DataFrame, window: int, step_bars: int) -> tuple[pd.Series, pd.Series]:
    ...
    for i in range(len(df)):
        ...
        block_start = (i // step_bars) * step_bars
        block_end = min(block_start + step_bars, len(df))

        lookback_start = max(0, block_start - window + step_bars)
        lookback_end = block_end

        if lookback_start < lookback_end:
            range_high.iloc[i] = high.iloc[lookback_start:lookback_end].max()
            range_low.iloc[i] = low.iloc[lookback_start:lookback_end].min()
        else:
            range_high.iloc[i] = high.iloc[i]
            range_low.iloc[i] = low.iloc[i]

    return range_high.ffill().astype(float), range_low.ffill().astype(float)
