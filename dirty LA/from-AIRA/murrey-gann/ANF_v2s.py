"""
ANF_v2s — Adaptive Nexus Forge, HMM Regime + Symmetric Long/Short (Freqtrade)
=============================================================================
EN: Symmetric variant of ANF_v2. Entry tiers, basic-safety bounds and the
    dynamic-leverage rules are mirror images for long and short, so the strategy
    treats both directions with equal logic (long buys oversold expecting a
    bounce; short sells overbought expecting a pullback — both mean-reversion).
    Use this fork when you want balanced behaviour in falling markets, not only
    in rising/ranging ones. Everything else (ML ensemble, HMM regime layer,
    persistence, audit, CLI) is identical to ANF_v2.
ES: Variante simétrica de ANF_v2. Los tiers de entrada, los límites de
    basic-safety y las reglas de leverage dinámico son espejo para long y short,
    de modo que la estrategia trata ambas direcciones con la misma lógica (el
    long compra en sobreventa esperando rebote; el short vende en sobrecompra
    esperando corrección — ambos de reversión a la media). Usa este fork cuando
    quieras un comportamiento equilibrado también en mercados bajistas, no solo
    en alcistas/laterales. Todo lo demás (ensemble ML, capa de régimen HMM,
    persistencia, auditoría, CLI) es idéntico a ANF_v2.
...
EN: Dual-mode file. (1) As a Freqtrade strategy: `freqtrade trade --strategy
    ...
    writes a frozen user_data/ml_models/ANF_v2s/hmm_regime.pkl. Copy that file to
...
ES: Fichero de doble modo. (1) Como estrategia Freqtrade: `freqtrade trade
    ...
    escribe un user_data/ml_models/ANF_v2s/hmm_regime.pkl congelado. Copia ese
...
# user_data/ml_models/ANF_v2s/, completely isolated from an ANF instance running
...
class ANF_v2s(IStrategy):
    ...
    EN: HMM training has two modes. (1) Default: the bot fits the HMM on a rolling
        ...
        drop the .pkl into user_data/ml_models/ANF_v2s/. A frozen model is loaded
    ...
    ES: El entrenamiento del HMM tiene dos modos. (1) Por defecto: el bot ajusta
        ...
        user_data/ml_models/ANF_v2s/. Un modelo congelado se carga al arrancar y
    ...
    def bot_start(self, **kwargs) -> None:
        ...
        try:
            ...
            if is_backtest_or_hyperopt:
                logger.warning(
                    "[ANF_v2s] BACKTEST/HYPEROPT detected (runmode=%s). ML training "
                ...
                logger.info(f"[ANF_v2s] bot_start: engine attached, runmode={runmode_raw}, "
                           f"ML training enabled")
        except Exception as e:
            logger.warning(f"[ANF_v2s] bot_start: failed to initialise engine ({e}); "
        ...
        logger.info(f"[ANF_v2s] ANF root dir: {self._anf_root.resolve()}")
        logger.info(f"[ANF_v2s] Audit trigger watching: {self._audit_trigger_path}")
            ...
            for trade in open_trades:
                ...
                if payload:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"[ANF_v2s] decision context logged for trade #{trade.id} "
                            f"{trade.pair}: {payload}")
                    except Exception as e:
                        logger.debug(
                            f"[ANF_v2s] could not persist decision_context "
                            f"for trade #{trade.id}: {e}")
        except Exception as e:
            # Decision logging must never break the bot loop.
            logger.debug(f"[ANF_v2s] decision-context persistence skipped: {e}")
    ...
    def _send_telegram_safe(self, text: str) -> None:
        ...
        try:
            ...
            logger.debug(f"[ANF_v2s] send_msg skipped: {e}")
    ...
    def _spawn_audit_subprocess(self) -> None:
        ...
        try:
            ...
            logger.warning(f"[ANF_v2s] could not locate strategy file for audit: {e}")
            self._send_telegram_safe(
        ...
        logger.info(f"[ANF_v2s] launching audit subprocess: {' '.join(cmd)}")
        self._send_telegram_safe(
        ...
        try:
            ...
            logger.info(f"[ANF_v2s] audit subprocess PID={proc.pid}")
            self._send_telegram_safe(f"🔍 ANF audit PID={proc.pid}. "
                                     f"Results will appear in this chat when ready.")
        except Exception as e:
            logger.warning(f"[ANF_v2s] failed to launch audit subprocess: {e}")
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: datetime,
        ...
        try:
            ...
            logger.warning(f"[ANF_v2s] custom_exit error for {pair}: {e}")
            return None
    ...
    def leverage(self, pair: str, current_time: datetime, current_rate: float, proposed_leverage: float,
        ...
        if side == "long":
            ...
            # EN: Symmetric short branch (ANF_v2s). Mirror of the long rules:
            #     a short entry is "better" when the market is overbought, so
            #     HIGH RSI increases leverage and LOW RSI decreases it; a bearish
            #     MACD histogram increases it; price ABOVE the SMA (room to fall)
            #     decreases it. The volatility brake (ATR) is direction-neutral
            #     and applies identically.
            # ES: Rama short simétrica (ANF_v2s). Espejo de las reglas long: una
            #     entrada short es "mejor" en sobrecompra, así que un RSI ALTO
            #     aumenta el leverage y uno BAJO lo reduce; un histograma MACD
            #     bajista lo aumenta; el precio POR ENCIMA de la SMA lo reduce.
            #     El freno por volatilidad (ATR) es neutro y se aplica igual.
            if current_rsi > self.leverage_rsi_high.value:
                base_leverage *= self.leverage_long_increase_factor.value
            elif current_rsi < self.leverage_rsi_low.value:
                base_leverage *= self.leverage_long_decrease_factor.value
    ...
    def populate_entry_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        basic_safety_short = (
            (dataframe['volume'] > 0) &
            (dataframe['rsi'] > 25) & (dataframe['rsi'] < 80) &
    ...
    def _daily_loss_blocked(self, current_time: datetime) -> bool:
        ...
        try:
            ...
            if today_pnl_abs <= threshold_abs:
                ...
                _log_info_throttled(
                    f"[ANF_v2s][KILL] Daily loss threshold hit "
            ...
            logger.debug(f"[ANF_v2s] daily-loss check error (failing open): {e}")
            return False
        ...
        try:
            ...
            for trade in open_trades:
                ...
                if abs(corr) > self.corr_threshold:
                    ...
                    _log_info_throttled(
                        f"[ANF_v2s] BLOCKED {direction} {pair} — correlated {corr:.2f} "
            ...
            logger.warning(f"[ANF_v2s] correlation filter error for {pair}: {e}; "
                           f"allowing trade")
            return True
...
def _cli_main(argv=None):
    ...
    ap.add_argument('--strategy-name', default='ANF_v2s',
...
def _train_hmm_standalone(years=3, pair='BTC/USDT:USDT', timeframe='1h',
    ...
    EN: Standalone trainer. Fetches `years` of OHLCV for the anchor pair via
        ...
        user_data/ml_models/ANF_v2s/ directory.
    ES: Entrenador standalone. Descarga `years` de OHLCV del par ancla vía ccxt,
        ...
        user_data/ml_models/ANF_v2s/ del VPS.
    """
    import sys as _sys
    if not HMM_AVAILABLE:
    ...
    print("[train-hmm] copy this .pkl to your VPS user_data/ml_models/ANF_v2s/")
    return 0
