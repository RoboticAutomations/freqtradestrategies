...
class NOTankAi_15(IStrategy):
    ...
    trailing_stop_positive_offset = 0.03  # Adjust as needed
...
# Add the confirm_trade_entry method
    
def confirm_trade_entry(self, pair: str, current_price: float) -> bool:
    dataframe, _ = self.dp.get_analyzed_dataframe(pair, self.timeframe)

    # Momentum-based entry
    momentum = ta.MOM(dataframe, timeperiod=10).iloc[-1]
    if momentum > 0:
        # Trend-based filter: only enter if price is above the 50-period EMA
        ema = ta.EMA(dataframe, timeperiod=50).iloc[-1]
        if current_price > ema:
            # Volume-based entry filter: enter only if recent volume is 2x the average
            recent_volume = dataframe['volume'].iloc[-1]
            avg_volume = dataframe['volume'].rolling(10).mean().iloc[-1]
    ...
    def confirm_trade_entry(self, pair: str, current_price: float) -> bool:
        ...
        # Momentum-based entry
        momentum = ta.MOM(dataframe, timeperiod=10).iloc[-1]
        if momentum > 0:  # Only enter if there's positive momentum
            return True
        
        # Volume-based entry filter
        recent_volume = dataframe['volume'].iloc[-1]
        avg_volume = dataframe['volume'].rolling(10).mean().iloc[-1]
        if recent_volume > 1.5 * avg_volume:  # Enter only if volume is 50% higher than average
            return True
...
initial_safety_order_trigger = DecimalParameter(low=-0.02, high=-0.01, default=-0.015, decimals=3, space="entry", optimize=True)
safety_order_step_scale = DecimalParameter(low=1.2, high=2.0, default=1.5, decimals=2, space="entry", optimize=True)
 DecimalParameter(low=-0.02, high=-0.01, default=-0.015, decimals=3, space="entry", optimize=True)
 DecimalParameter(low=-0.01, high=-0.005, default=-0.007, decimals=3, space="entry", optimize=True)

    
    max_safety_orders = IntParameter(1, 6, default=2, space="entry", optimize=True)
    safety_order_step_scale = DecimalParameter(low=1.05, high=1.25, default=1.1, decimals=2, space="entry", optimize=True)

    safety_order_volume_scale = DecimalParameter(low=1.1, high=1.5, default=1.3, decimals=1, space="entry", optimize=True)
...
minimal_roi = {
    "0": 0.07,   # 7% return after 0 minutes
    "60": 0.05,  # 5% return after 60 minutes
    "120": 0.03  # 3% return after 120 minutes
}
 {
    "0": 0.05,   # 5% return after 0 minutes
    "60": 0.04,  # 4% return after 60 minutes
    "120": 0.02  # 2% return after 120 minutes
}
 {
    "0": 0.03,   # 3% return after 0 minutes
    "60": 0.02,  # 2% return after 60 minutes
    "120": 0.01  # 1% return after 120 minutes
}
...
def confirm_trade_exit(self, pair: str, trade: Trade, current_price: float) -> bool:
    dataframe, _ = self.dp.get_analyzed_dataframe(pair, self.timeframe)

    # Loosen ATR-based trailing stop to 2 * ATR
    atr = ta.ATR(dataframe, timeperiod=14).iloc[-1]
    trailing_stop_price = current_price - 2 * atr  # Looser trailing stop
    ...
    atr = ta.ATR(dataframe, timeperiod=14).iloc[-1]
    trailing_stop_price = current_price - 1.5 * atr  # Adjust trailing stop based on ATR
    if current_price <= trailing_stop_price:
        return True
...
def confirm_trade_exit(self, pair: str, trade: Trade, current_price: float) -> bool:
    ...
    def confirm_trade_exit(self, pair: str, trade: Trade, order_type: str, amount: float, rate: float, 
        ...
        # 1. Fixed take-profit logic (% profit target)
        fixed_profit_target = 0.07  # 7% profit target
        entry_price = trade.open_rate
        target_price = entry_price * (1 + fixed_profit_target)
        if rate >= target_price:
            return True
        
        # 3. ATR-based trailing stop logic
        atr = ta.ATR(dataframe, timeperiod=14).iloc[-1]
        trailing_stop_price = rate - 1.5 * atr  # Exit if price drops by 2 ATR units
        if rate <= trailing_stop_price:
            return True

        # 5. Volume spike exit logic
        recent_volume = dataframe['volume'].iloc[-1]
        avg_volume = dataframe['volume'].rolling(10).mean().iloc[-1]
        if recent_volume > 2 * avg_volume:  # Exit if recent volume is 2x the rolling average volume
 
         # Add Moving Average (SMA) exit logic
        ma = ta.SMA(dataframe, timeperiod=50).iloc[-1]  # 50-period SMA
        if rate < ma:  # Exit if the price drops below the 50-period SMA
            return True
