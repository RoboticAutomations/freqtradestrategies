...
class TankAi15Modulus(IStrategy):
    ...
    safety_order_reserve = DecimalParameter(1, 10, default=1.5, space="buy", optimize=True)

    ### Custom Functions
    # Modulus
    peaks = IntParameter(
        70, 100, default=80, space="buy", optimize=True
    )  ### initial smallest window
    bull_bear = IntParameter(120, 250, default=180, space="buy", optimize=True)
    trend = DecimalParameter(
        low=25, high=40, default=29.6, decimals=1, space="buy", optimize=True, load=True
    )
    volatility = DecimalParameter(
        low=30, high=50, default=38.4, decimals=1, space="buy", optimize=True, load=True
    )
    ...
    tsl_target5 = DecimalParameter(
        low=0.3, high=0.4, default=0.3, decimals=2, space="sell", optimize=True, load=True
    )
    ts5 = DecimalParameter(
        low=0.04, high=0.06, default=0.05, decimals=2, space="sell", optimize=True, load=True
    )
    tsl_target4 = DecimalParameter(
        low=0.18, high=0.3, default=0.2, decimals=2, space="sell", optimize=True, load=True
    )
    ts4 = DecimalParameter(
        low=0.03, high=0.05, default=0.045, decimals=3, space="sell", optimize=True, load=True
    )
    tsl_target3 = DecimalParameter(
        low=0.12, high=0.18, default=0.15, decimals=2, space="sell", optimize=True, load=True
    )
    ts3 = DecimalParameter(
        low=0.025, high=0.04, default=0.035, decimals=3, space="sell", optimize=True, load=True
    )
    tsl_target2 = DecimalParameter(
        low=0.07, high=0.12, default=0.1, decimals=2, space="sell", optimize=True, load=True
    )
    ts2 = DecimalParameter(
        low=0.015, high=0.03, default=0.02, decimals=2, space="sell", optimize=True, load=True
    )
    tsl_target1 = DecimalParameter(
        low=0.04, high=0.07, default=0.06, decimals=3, space="sell", optimize=True, load=True
    )
    ts1 = DecimalParameter(
        low=0.01, high=0.016, default=0.013, decimals=3, space="sell", optimize=True, load=True
    )
    tsl_target0 = DecimalParameter(
        low=0.02, high=0.05, default=0.040, decimals=3, space="sell", optimize=True, load=True
    )
    ts0 = DecimalParameter(
        low=0.008, high=0.015, default=0.012, decimals=3, space="sell", optimize=True, load=True
    )
    ...
    plot_config = {
        "main_plot": {
            "200sma": {"color": "#eec83f", "type": "line"},
            "enter_tag": {"color": "#3ab61a"},
            "exit_tag": {"color": "#6e16a4"},
        },
        "subplots": {
            "AI": {
                "do_predict": {"color": "#4f227c", "type": "bar"},
                "minima": {"color": "#2a643c", "type": "line"},
                "maxima": {"color": "#e277b7", "type": "line"},
                "minima_check": {"color": "#2a643c", "type": "line"},
                "maxima_check": {"color": "#e277b7", "type": "line"},
            },
            "order": {"order": {"color": "#20ba5f", "type": "line"}},
            "max/min": {
                "max": {"color": "#8d299", "type": "line"},
                "atr_pcnt": {"color": "#30e7da", "type": "line"},
                "min": {"color": "#19f63a", "type": "line"},
            },
            "ATR Pcnt": {
                "atr_pcnt": {"color": "#2187f0", "type": "line"},
                "atr_threshold": {"color": "#d9cd6c", "type": "line"},
            },
        },
    }
    ...
    def custom_stake_amount(
        ...
        **kwargs,
        ...
        proposed_stake = proposed_rate / (self.max_dca_multiplier + self.safety_order_reserve.value)
    ...
    def adjust_trade_position(
        ...
        **kwargs,
        ...
        if current_profit is not None:
            logger.info(
                f"{trade.pair} - Current Profit: {current_profit} # of Entries: {trade.nr_of_successful_entries}"
            )
    ...
    def custom_stake_amount(
        ...
        **kwargs,
        ...
        proposed_stake = proposed_stake / (
            self.max_dca_multiplier + self.safety_order_reserve.value
    ...
    def set_freqai_targets(self, dataframe, **kwargs):
        ...
        dataframe["sma_pc"] = (
            abs((dataframe["sma"] - dataframe["sma"].shift()) / dataframe["sma"]) * 100
        )
        dataframe["atr_pcnt"] = (qtpylib.atr(dataframe, window=self.atr.value)) / dataframe[
            "ha_close"
        ]
        dataframe["modulation"] = (
            1
            + (dataframe["sma_pc"] * self.trend.value)
            + (dataframe["atr_pcnt"] * self.volatility.value)
        )
        ...
        dataframe["order"] = (
            (dataframe["modulation"] * self.peaks.value)
            .round()
            .fillna(self.bull_bear.value)
            .astype(int)
        )
        dataframe["order"] = np.where(
            dataframe["order"] > max_window, max_window, dataframe["order"]
        )
        dataframe["order"] = np.where(
            dataframe["order"] < min_window, min_window, dataframe["order"]
        )

        order = (
            dataframe["order"].iloc[-1] if not dataframe["order"].empty else self.bull_bear.value
        )
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        df.loc[
            ...
            & (df["volume"] > 0),  # Make sure Volume is not 0
            ["enter_long", "enter_tag"],
        ] = (1, "Minima Check Predicted Max Order")
        ...
        df.loc[
            ...
            & (df["volume"] > 0),  # Make sure Volume is not 0
            ["enter_long", "enter_tag"],
        ] = (1, "Minima Check Predicted Mid Order")
        ...
        df.loc[
            ...
            & (df["volume"] > 0),  # Make sure Volume is not 0
            ["enter_long", "enter_tag"],
        ] = (1, "Minima Check Predicted Min Order")
        # short
        df.loc[
            (df["do_predict"] == 0)
            & (df["maxima_check"] == 0)
        ...
        df.loc[
            ...
            & (df["volume"] > 0),  # Make sure Volume is not 0
            ["enter_short", "enter_tag"],
        ] = (1, "Maxima Check Predicted Max Order")
        ...
        df.loc[
            ...
            & (df["volume"] > 0),  # Make sure Volume is not 0
            ["enter_short", "enter_tag"],
        ] = (1, "Maxima Check Predicted Mid Order1")
        ...
        df.loc[
            ...
            & (df["volume"] > 0),  # Make sure Volume is not 0
            ["enter_short", "enter_tag"],
        ] = (1, "Maxima Check Predicted Min Order2")
    ...
    def populate_exit_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        df.loc[
            ...
            & (df["volume"] > 0),  # Make sure Volume is not 0
            ["exit_short", "exit_tag"],
        ] = (1, "Minima Check Predicted Mid Order1")
        ...
        df.loc[
            ...
            & (df["volume"] > 0),  # Make sure Volume is not 0
            ["exit_short", "exit_tag"],
        ] = (1, "Minima Check Predicted Min Order2")
