...
class Tank5HurstDCAV1(IStrategy):
    ...
    max_dca_multiplier = DecimalParameter(low=1.1, high=1.5, default=1.1, decimals=1 ,space='buy', optimize=True, load=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[dataframe['osc'] > 0, "osc_UP"] = dataframe['osc']
        dataframe.loc[dataframe['osc'] < 0, "osc_DN"] = dataframe['osc']
        dataframe['osc_UP'].ffill()
        dataframe['osc_DN'].ffill()
        dataframe['osc_MEAN_UP'] = dataframe['osc_UP'].mean() * 2
        dataframe['osc_MEAN_DN'] = dataframe['osc_DN'].mean() * 2
        ...
        # Calculate the half-span of the dominant cycle period
        half_span_period = cycle_period // 2

        # Apply an inverse half-span average to the dominant cycle
        ...
        dataframe['lowerspan_mean'] = dataframe['lowerspan'].rolling(int(harmonics[2])).mean()
        dataframe['upperspan_mean'] = dataframe['upperspan'].rolling(int(harmonics[2])).mean()
        dataframe['span avg'] = (dataframe['lowerspan'] + dataframe['upperspan']) /2
        dataframe['upper_std'] = dataframe['span avg'] + dataframe['upperspan'].std()
        dataframe['lower_std'] = dataframe['span avg'] - dataframe['lowerspan'].std()
        ...
        # Generate signals based on each significant period and harmonics
        ...
        # Normalize the 'signal' column for clarity
        dataframe['signal'] = dataframe['signal'] / dataframe['signal'].abs().max()

        # Filter based on power levels
        ...
        # Additional calculations for signal mean and EMA
        ...
        # # Ensure the DataFrame has no NaN values in essential columns
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        full_send2 = (
                ...
                (df['osc'] < df['osc_MEAN_DN']) & 
                (df['lowerspan_mean'] > df['lowerspan']) & 
                # (df['h0_move_mean'] < df['h2_move']) & 
                (df['upper_std'] < df['lowerspan']) & 
                (df['signal'] > df['signal_MEAN_UP']) &
                (df['volume'] > 0)   # Make sure Volume is not 0
        )
        ...
        full_send5 = (
                ...
                (df['dominant_move_soon'].shift() == False) &
                (df['dominant_move_soon'] == True) &
                (df['signal'] > 0) & 
                (df['volume'] > 0)   # Make sure Volume is not 0

        )
        df.loc[full_send5, 'enter_long'] = 1
        df.loc[full_send5, 'enter_tag'] = 'Dormant Cycle Expired'
        ...
        dump_over7 = (              
                ...
                (df["lowerspan"] < df['upperspan']) &
                (df['lowerspan'].shift() > df['lowerspan']) &
                (df['h1_move_mean'] < df['h2_move']) & 
                (df["lowerspan"] < df['span_h2_limit']) &
                (df['signal_ma'] < df['signal_MEAN_UP']) &
                (df['volume'] > 0)   # Make sure Volume is not 0

        )
        df.loc[dump_over7, 'enter_long'] = 1
        df.loc[dump_over7, 'enter_tag'] = 'Dump Over 7'
    ...
    def populate_exit_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        profit_taker1 = (
                ...
                (df['harmonic_1/4'] < df['harmonic_1/3']) &
                (df['dominant_cycle'] > df['harmonic_1/4']) &  
                (df['volume'] > 0)   # Make sure Volume is not 0
...
# Function to calculate upper and lower envelopes
