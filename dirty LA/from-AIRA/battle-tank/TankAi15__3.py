...
class TankAi15(IStrategy):
    ...
    max_safety_orders = IntParameter(1, 6, default=3, space='buy', optimize=True)
    ...
    tsl_target6 = DecimalParameter(low=0.45, high=0.55, default=0.51, decimals=2, space='sell', optimize=True, load=True)
    ts6 = DecimalParameter(low=0.05, high=0.07, default=0.06,decimals=2, space='sell', optimize=True, load=True)
    tsl_target5 = DecimalParameter(low=0.4, high=0.5, default=0.46, decimals=2, space='sell', optimize=True, load=True)
    ts5 = DecimalParameter(low=0.04, high=0.06, default=0.05,decimals=2, space='sell', optimize=True, load=True)
    tsl_target4 = DecimalParameter(low=0.36, high=0.44, default=0.41, decimals=2, space='sell', optimize=True, load=True)
    ts4 = DecimalParameter(low=0.04, high=0.05, default=0.045, decimals=3, space='sell', optimize=True, load=True)
    tsl_target3 = DecimalParameter(low=0.27, high=0.33, default=0.31, decimals=2,space='sell', optimize=True, load=True)
    ts3 = DecimalParameter(low=0.03, high=0.04, default=0.035, decimals=3,space='sell', optimize=True, load=True)
    tsl_target2 = DecimalParameter(low=0.17, high=0.23, default=0.21, decimals=2,space='sell', optimize=True, load=True)
    ts2 = DecimalParameter(low=0.025, high=0.035, default=0.03, decimals=3,space='sell', optimize=True, load=True)
    tsl_target1 = DecimalParameter(low=0.08, high=0.12, default=0.11, decimals=2,space='sell', optimize=True, load=True)
    ts1 = DecimalParameter(low=0.015, high=0.03, default=0.02, decimals=2,space='sell', optimize=True, load=True)
    tsl_target0 = DecimalParameter(low=0.04, high=0.06, default=0.055, decimals=3,space='sell', optimize=True, load=True)
    ts0 = DecimalParameter(low=0.01, high=0.016, default=0.013, decimals=3,space='sell', optimize=True, load=True)
    ...
    def custom_stake_amount(self, pair: str, current_time: datetime, current_rate: float,
                            ...
                            **kwargs) -> float:
        """
        Determine the custom stake amount for the initial order.
        """
    ...
    def custom_entry_price(self, pair: str, trade: Optional['Trade'], current_time: datetime, proposed_rate: float,
        ...
        if self.last_entry_price is not None and abs(entry_price - self.last_entry_price) < 0.001:  # Tolerance for floating-point comparison
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float, current_profit: float, **kwargs):
        if current_profit <= -0.85:  # Se la perdita è inferiore al -85%
            logger.info(f'{trade.pair} Loss is greater than 85%. Exiting position.')
            self.dp.send_msg(f'{trade.pair} Loss is greater than 85%. Exiting position.')
            return True  # Chiudi la posizione
        return False  # Altrimenti, mantieni la posizione aperta
    ...
    def custom_stoploss(self, pair: str, trade: 'Trade', current_time: datetime,
        ...
        if trade_duration > 0:
            if (current_profit > self.tsl_target6.range):
                self.dp.send_msg(f'*** {pair} *** Profit: {current_profit} - lvl5 {stop5}/{stop5a} activated')
                return self.ts6.range 
        if trade_duration > 15:
            if (current_profit > self.tsl_target5.range):
                self.dp.send_msg(f'*** {pair} *** Profit: {current_profit} - lvl5 {stop5}/{stop5a} activated')
                return self.ts5.range
        if trade_duration > 30: 
            if (current_profit > self.tsl_target4.range):
                self.dp.send_msg(f'*** {pair} *** Profit {current_profit} - lvl4 {stop4}/{stop4a} activated')
                return self.ts4.range 
        if trade_duration > 60:
            if (current_profit > self.tsl_target3.range):
                self.dp.send_msg(f'*** {pair} *** Profit {current_profit} - lvl3 {stop3}/{stop3a} activated')
                return self.ts3.range
        if trade_duration > 90: 
            if (current_profit > self.tsl_target2.range):
                self.dp.send_msg(f'*** {pair} *** Profit {current_profit} - lvl2 {stop2}/{stop2a} activated')
                return self.ts2.range
        if trade_duration > 120:
            if (current_profit > self.tsl_target1.range):
                self.dp.send_msg(f'*** {pair} *** Profit {current_profit} - lvl1 {stop2}/{stop2a} activated')
                return self.ts1.range
        if trade_duration > 180:
            if (current_profit > self.tsl_target0.range):
                self.dp.send_msg(f'*** {pair} *** Profit {current_profit} - lvl0 {stop2}/{stop2a} activated')
                return self.ts0.range
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        """
        Adjust the trade position based on the current profit and the number of successful entries.
        """
        ...
        # Check if max number of safety orders is reached
        ...
        try:
            ...
            print(f"Error adjusting trade position: {exception}")
            return None
    ...
    def leverage(self, pair: str, current_time: 'datetime', current_rate: float,
        ...
        long_increase_factor = 22  # Increase  the base leverage for long positions
        long_decrease_factor = 12  # Decrease  the base leverage for long positions
        short_increase_factor = 22  # Increase  base leverage for short positions
        short_decrease_factor = 12  # Decrease  the base leverage for short positions
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['HCL3'] = (dataframe['high'] + dataframe['close'] + dataframe['low']) / 3
        dataframe['rsx'] = pta.rsx(dataframe['HCL3'],length=14, drift=1, offset=0)
        stc = tastc.trend.stc(dataframe['close'], window_slow=120, window_fast=18, cycle=12)
        dataframe["stc5"] = stc
        ...
        # CCI
        dataframe['cci2'] = ta.CCI(dataframe, timeperiod=170)
        dataframe['cci1'] = ta.CCI(dataframe, timeperiod=34)
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        df.loc[
            ...
            ['enter_long', 'enter_tag']] = (1, 'Minima Full Send2')
        ...
        df.loc[
            ...
            ['enter_short', 'enter_tag']] = (1, 'Maxima Full Send2')
    ...
    def populate_exit_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        df.loc[
            ...
            ['exit_long', 'exit_tag']] = (1, 'Maxima Check Complete')
