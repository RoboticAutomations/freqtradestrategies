...
class haFbmVVRP(IStrategy):
    ...
    max_epa = IntParameter(0, 2, default = 1 ,space='buy', optimize=True, load=True) # of additional buys.
    max_dca_multiplier = DecimalParameter(low=1.0, high=1.2, default=1.1, decimals=1 ,space='buy', optimize=True, load=True)
    use_static = BooleanParameter(default=True, space="buy", optimize=True, load=True)
    filldelay = IntParameter(5, 300, default = 100 ,space='buy', optimize=True, load=True)
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        # Take Profit if m00n
        if current_profit > TP2 and trade.nr_of_successful_exits == 0 and bias < 0:
