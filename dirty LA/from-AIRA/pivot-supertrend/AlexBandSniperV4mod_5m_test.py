...
class AlexBandSniperV4(IStrategy):
    ...
    # Use current 5m data to simulate 1h trend (12x)
    try:
        dataframe['ema50_1h_1h'] = ta.EMA(dataframe, timeperiod=600)   # 50 * 12
        dataframe['ema200_1h_1h'] = ta.EMA(dataframe, timeperiod=2400) # 200 * 12
        dataframe['trend_1h_1h'] = ta.EMA(dataframe, timeperiod=252)   # 21 * 12
        dataframe['trend_strength_1h_1h'] = ta.ADX(dataframe)
        dataframe['rsi_1h_1h'] = ta.RSI(dataframe, timeperiod=168)     # 14 * 12
